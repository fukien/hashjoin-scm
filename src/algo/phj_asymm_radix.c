...
extern int cur_t;
typedef void (*asymm_radix_joinfunction_t)(const relation_t * const, const relation_t * const, relation_t * const, join_arg_t * const);

/** holds the arguments passed to each thread */
typedef struct phj_asymm_radix_arg_t {
	int _tid;
	...
	asymm_radix_joinfunction_t joinfunction;
	...
	void *intermediate;
} phj_asymm_radix_arg_t;


typedef struct asymm_radix_part_t {
	int _tid;
	pthread_barrier_t *barrier;
...
void asymm_radix_parallel_radix_partition(asymm_radix_part_t * const part, const size_t padding_num) {
	int rv;
...
#ifdef USE_SWWCB_OPTIMIZED_PART
void asymm_radix_parallel_radix_partition_optimized(asymm_radix_part_t * const part, swwcb_t * const swwcb, 
...
void asymm_radix_radix_cluster(relation_t out_rel, relation_t in_rel, size_t *hist, 
...
void asymm_radix_radix_cluster_optimized(relation_t out_rel, relation_t in_rel, size_t *hist, const size_t fanout, 
...
void asymm_radix_serial_radix_partition(task_t * const task, task_queue_t * join_queue, const size_t fanout, const key_t_ bitskip, 
	...
	asymm_radix_radix_cluster_optimized(task->r_tmp, task->r_rel, r_hist, fanout, bitskip, swwcb, padding_num, task->r_global_offset);
#else /* USE_SWWCB_OPTIMIZED_RADIX_CLUSTER_ON_NVM */
	asymm_radix_radix_cluster(task->r_tmp, task->r_rel, r_hist, fanout, bitskip, padding_num);
	...
	for (size_t idx = 0; idx < fanout; idx ++) {
		if (r_hist[idx*2] > 0) {
			...
			*max_build_side_tup_num = *max_build_side_tup_num > r_hist[idx*2] ? *max_build_side_tup_num : r_hist[idx*2];
...
void asymm_radix_bc_join(const relation_t * const r_rel, const relation_t * const s_rel, relation_t * const r_tmp, join_arg_t * const join_arg) {
...
void asymm_radix_sc_join(const relation_t * const r_rel, const relation_t * const s_rel, relation_t * const r_tmp, join_arg_t * const join_arg) {
	join_arg -> matched_cnt = 0;
	join_arg -> checksum = 0;
...
void asymm_radix_hr_join(const relation_t * const r_rel, const relation_t * const s_rel, relation_t * const r_tmp, join_arg_t * const join_arg) {
	join_arg -> matched_cnt = 0;
	join_arg -> checksum = 0;
...
void asymm_radix_hro_join(const relation_t * const r_rel, const relation_t * const s_rel, relation_t * const r_tmp, join_arg_t * const join_arg) {
	join_arg -> matched_cnt = 0;
	join_arg -> checksum = 0;
...
void *prj_asymm_radix_thread_buildpart(void *param) {
	phj_asymm_radix_arg_t *phj_arg = (phj_asymm_radix_arg_t *) param;
	...
	r_output_offset = (size_t *) new_alloc_nvm( sizeof(size_t) * FANOUT_PASS1_ASYMM);
	s_output_offset = (size_t *) new_alloc_nvm( sizeof(size_t) * FANOUT_PASS1_ASYMM);
#else /* OUTPUT_OFFSET_NVM */
	r_output_offset = (size_t *) alloc_dram( sizeof(size_t) * FANOUT_PASS1_ASYMM, CACHELINE_SIZE);
	s_output_offset = (size_t *) alloc_dram( sizeof(size_t) * FANOUT_PASS1_ASYMM, CACHELINE_SIZE);
	...
	swwcb = (swwcb_t *) new_alloc_nvm(sizeof(swwcb_t) * FANOUT_PASS1_ASYMM);
#else /* USE_SWWCB_OPTIMIZED_PART_ON_NVM */
	swwcb = (swwcb_t *) alloc_aligned_dram(sizeof(swwcb_t) * FANOUT_PASS1_ASYMM, CACHELINE_SIZE);
	...
	asymm_radix_parallel_radix_partition_optimized(&part, swwcb, FANOUT_PASS2_ASYMM * PADDING_UNIT_NUM, 0);
#else /* USE_SWWCB_OPTIMIZED_PART */
	asymm_radix_parallel_radix_partition(&part, FANOUT_PASS2_ASYMM * PADDING_UNIT_NUM);
#endif /* USE_SWWCB_OPTIMIZED_PART */
...
#if NUM_PASSES == 2
	size_t *ser_r_hist = (size_t *) alloc_memory( sizeof(size_t) * 2 * FANOUT_PASS2_ASYMM, CACHELINE_SIZE);
	size_t *ser_s_hist = (size_t *) alloc_memory( sizeof(size_t) * 2 * FANOUT_PASS2_ASYMM, CACHELINE_SIZE);

	memset_localize(ser_r_hist, sizeof(size_t) * 2 * FANOUT_PASS2_ASYMM);
	memset_localize(ser_s_hist, sizeof(size_t) * 2 * FANOUT_PASS2_ASYMM);

	void *ser_swwcb = NULL;
#ifdef USE_SWWCB_OPTIMIZED_RADIX_CLUSTER
#ifdef USE_SWWCB_OPTIMIZED_RADIX_CLUSTER_ON_NVM
	ser_swwcb = new_alloc_nvm(sizeof(swwcb_t) * FANOUT_PASS2_ASYMM);
	pmem_memset_persist(ser_swwcb, 0, sizeof(swwcb_t) * FANOUT_PASS2_ASYMM);
#else /* USE_SWWCB_OPTIMIZED_RADIX_CLUSTER_ON_NVM */
	ser_swwcb = alloc_aligned_dram(sizeof(swwcb_t) * FANOUT_PASS2_ASYMM, CACHELINE_SIZE);
	memset(ser_swwcb, 0, sizeof(swwcb_t) * FANOUT_PASS2_ASYMM);
#endif /* USE_SWWCB_OPTIMIZED_RADIX_CLUSTER_ON_NVM */
#endif /* USE_SWWCB_OPTIMIZED_RADIX_CLUSTER */
	...
	while ( ( task = task_queue_get_atomic(part_queue) ) ) {
		asymm_radix_serial_radix_partition(task, phj_arg->join_queue, 
			FANOUT_PASS2_ASYMM, NUM_RADIX_BITS_PASS1, 
...
#ifdef USE_SWWCB_OPTIMIZED_RADIX_CLUSTER
#ifdef USE_SWWCB_OPTIMIZED_RADIX_CLUSTER_ON_NVM
	pmem_unmap(ser_swwcb, sizeof(swwcb_t) * FANOUT_PASS2_ASYMM);
#else /* USE_SWWCB_OPTIMIZED_RADIX_CLUSTER_ON_NVM */
	free(ser_swwcb);
#endif /* USE_SWWCB_OPTIMIZED_RADIX_CLUSTER_ON_NVM */
#endif /* USE_SWWCB_OPTIMIZED_RADIX_CLUSTER */
	dealloc_memory(ser_r_hist, sizeof(size_t) * 2 * FANOUT_PASS2_ASYMM);
	dealloc_memory(ser_s_hist, sizeof(size_t) * 2 * FANOUT_PASS2_ASYMM);
#endif /* NUM_PASSES == 2 */
	...
	size_t *skew_hand_r_output_offset;

#ifdef OUTPUT_OFFSET_NVM
	skew_hand_r_output_offset = (size_t *) new_alloc_nvm( sizeof(size_t) * FANOUT_PASS2_ASYMM );
#else /* OUTPUT_OFFSET_NVM */
	skew_hand_r_output_offset = (size_t *) alloc_dram( sizeof(size_t) * FANOUT_PASS2_ASYMM, CACHELINE_SIZE);
#endif /* OUTPUT_OFFSET_NVM */

#ifdef USE_SWWCB_OPTIMIZED_PART
	swwcb_t *skew_hand_swwcb;
#ifdef USE_SWWCB_OPTIMIZED_PART_ON_NVM
	skew_hand_swwcb = (swwcb_t *) new_alloc_nvm(sizeof(swwcb_t) * FANOUT_PASS2_ASYMM);
	pmem_memset_persist(skew_hand_swwcb, 0, sizeof(swwcb_t) * FANOUT_PASS2_ASYMM);
#else /* USE_SWWCB_OPTIMIZED_PART_ON_NVM */
	skew_hand_swwcb = (swwcb_t *) alloc_aligned_dram(sizeof(swwcb_t) * FANOUT_PASS2_ASYMM, CACHELINE_SIZE);
	memset(skew_hand_swwcb, 0, sizeof(swwcb_t) * FANOUT_PASS2_ASYMM);
#endif /* USE_SWWCB_OPTIMIZED_PART_ON_NVM */
#endif /* USE_SWWCB_OPTIMIZED_PART */
	...
	while(1) {
...
#ifdef OUTPUT_OFFSET_NVM
		pmem_memset_persist(skew_hand_r_output_offset, 0, sizeof(size_t) * FANOUT_PASS2_ASYMM );
#else /* OUTPUT_OFFSET_NVM */
		memset(skew_hand_r_output_offset, 0, sizeof(size_t) * FANOUT_PASS2_ASYMM );
#endif /* OUTPUT_OFFSET_NVM */
		...
		asymm_radix_parallel_radix_partition_optimized(&part, skew_hand_swwcb, PADDING_UNIT_NUM, (*skewtask)->r_global_offset);

#else /* USE_SWWCB_OPTIMIZED_PART */
		asymm_radix_parallel_radix_partition(&part, PADDING_UNIT_NUM);
#endif /* USE_SWWCB_OPTIMIZED_PART */
...
#ifdef USE_SWWCB_OPTIMIZED_PART
#ifdef USE_SWWCB_OPTIMIZED_PART_ON_NVM
	pmem_unmap(skew_hand_swwcb, sizeof(swwcb_t) * FANOUT_PASS2_ASYMM);
#else /* USE_SWWCB_OPTIMIZED_PART_ON_NVM */
...
#ifdef OUTPUT_OFFSET_NVM
	pmem_unmap(skew_hand_r_output_offset, sizeof(size_t) * FANOUT_PASS2_ASYMM);
#else /* OUTPUT_OFFSET_NVM */
	dealloc_dram(skew_hand_r_output_offset, sizeof(size_t) * FANOUT_PASS2_ASYMM);
#endif /* OUTPUT_OFFSET_NVM */
...
void *prj_asymm_radix_thread_probejoin(void *param) {
	phj_asymm_radix_arg_t *phj_arg = (phj_asymm_radix_arg_t *) param;
	join_arg_t join_arg;
...
void asymm_radix_join_init_run(const datameta_t datameta, timekeeper_t * const timekeeper, 
	...
	phj_asymm_radix_arg_t args[max_thread_num];
	...
	for (int idx = 0; idx < BUILDPART_THREAD_NUM; idx ++) {
		...
		rv = pthread_create(&threadpool[idx], &attr, prj_asymm_radix_thread_buildpart, args+idx);
		if (rv){
	...
	for (int idx = 0; idx < PROBEJOIN_THREAD_NUM; idx ++) {
		...
		rv = pthread_create(&threadpool[idx], &attr, prj_asymm_radix_thread_probejoin, args+idx);
		if (rv){
			printf("ERROR; return code from pthread_create() is %d\n", rv);
			exit(EXIT_FAILURE);
		}
...
void phj_asymm_radix_bc(const datameta_t datameta, timekeeper_t * const timekeeper) {
	asymm_radix_join_init_run(datameta, timekeeper, asymm_radix_bc_join);
}

void phj_asymm_radix_sc(const datameta_t datameta, timekeeper_t * const timekeeper) {
	asymm_radix_join_init_run(datameta, timekeeper, asymm_radix_sc_join);
}

void phj_asymm_radix_hr(const datameta_t datameta, timekeeper_t * const timekeeper) {
	asymm_radix_join_init_run(datameta, timekeeper, asymm_radix_hr_join);
}

#if TUPLE_T_SIZE <= AVX512_SIZE
void phj_asymm_radix_hro(const datameta_t datameta, timekeeper_t * const timekeeper) {
	asymm_radix_join_init_run(datameta, timekeeper, asymm_radix_hro_join);
}
#endif /* TUPLE_T_SIZE <= AVX512_SIZE */
