...
#include "../inc/task_pthread_ind.h"
...
#if NUM_PASSES != 1
static ind_part_buffer_t *r_shr_free_buffers_pass_2;
static ind_part_buffer_t *s_shr_free_buffers_pass_2;
#else /* NUM_PASSES != 1 */
#ifdef SKEW_HANDLING
static ind_part_buffer_t *r_shr_free_buffers_pass_2;
static ind_part_buffer_t *s_shr_free_buffers_pass_2;
...
extern int cur_t;
typedef void (*ind_joinfunction_t)(task_t * const, join_arg_t * const);


/** holds the arguments passed to each thread */
typedef struct phj_ind_arg_t {
	int _tid;
	...
	ind_part_buffer_t *r_part_rear;
	size_t r_pvt_buf_num;
	ind_part_buffer_t *r_1st_pvt_buf;

	relation_t s_rel;
	ind_part_buffer_t *s_part_buffer;
	ind_part_buffer_t *s_part_rear;
	size_t s_pvt_buf_num;
	ind_part_buffer_t *s_1st_pvt_buf;

#if NUM_PASSES != 1
	ind_part_buffer_t *r_2nd_pvt_buf;
	ind_part_buffer_t *s_2nd_pvt_buf;
#else /* NUM_PASSES != 1 */
#ifdef SKEW_HANDLING
	ind_part_buffer_t *r_2nd_pvt_buf;
	ind_part_buffer_t *s_2nd_pvt_buf;
	...
	// task_t ** task_1st_pass;
	// ind_part_buffer_t **r_head, **r_rear, **s_head, **s_rear;
#if SKEW_HANDLING
	task_queue_t *skew_queue;
	task_t ** skewtask;
#endif /* SKEW_HANDLING */

	ind_joinfunction_t joinfunction;
...
#ifdef SYNCSTATS
	synctimer_phj_ind_t *localtimer;
	synctimer_phj_ind_t *globaltimer;
#ifdef SKEW_HANDLING
	...
	bool **r_is_rear_flag, **s_is_rear_flag;

#ifdef SKEW_HANDLING
	size_t **skew_hand_r_part_hist, **skew_hand_s_part_hist;
	bool **skew_r_is_rear_flag, **skew_s_is_rear_flag;

	size_t **skew_hand_r_link_buffer_num, **skew_hand_s_link_buffer_num;
	size_t **skew_skew_r_link_buffer_num, **skew_skew_s_link_buffer_num;
#endif /* SKEW_HANDLING */
...
#ifdef SKEW_HANDLING
	size_t *skew_hand_link_buffer_num;
#endif /* SKEW_HANDLING */
} ind_part_t;


void ind_parallel_partition_first(ind_part_t * const part) {
	...
	ind_part_buffer_t *free_buffers;
	if (part->rel_flag == 0) {
	...
	for (size_t idx = 0; idx < part->rel.tuple_num; idx ++) {
		...
		if (cur->count == PART_BUFFER_T_CAPACITY) {
			if (rear_flag || nxt->count == PART_BUFFER_T_CAPACITY) {
				if (rear_flag) {
					new = part->part_rear + tmp_hashkey;
					*(part->is_rear_flag + tmp_hashkey) = true;
				} else if ( *part->pvt_buf_idx < part->pvt_buf_num ) {
					new = part->pvt_buf + (*part->pvt_buf_idx);
					(*part->pvt_buf_idx) ++;
				} else {
...
#ifdef SKEW_HANDLING
				part->skew_hand_link_buffer_num[tmp_hashkey] ++;
#endif /* SKEW_HANDLING */
...
#ifdef SKEW_HANDLING
void ind_parallel_partition_following(ind_skew_part_t *part) {
	...
	ind_part_buffer_t *tmp_in_buf = part->in_part;
...
#ifdef SKEW_HANDLING
				part->skew_skew_link_buffer_num[tmp_hashkey] ++;
#endif /* SKEW_HANDLING */
...
void ind_parallel_partition_optimized_first(ind_part_t * const part, swwcb_t * const swwcb) {
	key_t_ hashmask = (part->hashmask) << part->bitskip;
	key_t_ bitskip = part->bitskip;
	...
	for (size_t idx = 0; idx < part->rel.tuple_num; idx ++) {
		...
		tmp_swwcb[slot_mod] = part->rel.tuples[idx];
...
#ifdef SKEW_HANDLING
void ind_parallel_partition_optimized_following(ind_skew_part_t * const part, swwcb_t * const swwcb) {
	key_t_ hashmask = (part->hashmask) << part->bitskip;
	key_t_ bitskip = part->bitskip;
...
void ind_cluster(ind_cluster_arg_t *arg) {
	...
	ind_part_buffer_t *tmp_in_buf = arg->in_part;
		...
		for (size_t idx = 0; idx < tmp_in_buf->count; idx ++) {
			...
			if (cur->count == PART_BUFFER_T_CAPACITY) {
				if (!nxt || nxt->count == PART_BUFFER_T_CAPACITY) {
					if (*arg->pvt_buf_idx < arg->pvt_buf_num) {
						new = arg->pvt_buf + (*arg->pvt_buf_idx);
						(*arg->pvt_buf_idx) ++;
					} else {
						new = free_buffers + __sync_fetch_and_add(free_buffers_idx, 1);
					}
...
void ind_cluster_optimized(ind_cluster_arg_t *arg, swwcb_t * const swwcb) {
	key_t_ hashmask = (arg->fanout - 1) << (arg->bitskip);
	key_t_ bitskip = arg->bitskip;
...
void ind_serial_partition(task_t * const task, task_queue_t *join_queue, ind_serial_part_t *part) {

	memset_localize(part->r_hist, sizeof(size_t)*part->fanout);
	memset_localize(part->s_hist, sizeof(size_t)*part->fanout);

	ind_cluster_arg_t ind_cluster_arg;
	ind_cluster_arg.fanout = part->fanout;
	ind_cluster_arg.bitskip = part->bitskip;

#ifdef USE_SWWCB_OPTIMIZED_RADIX_CLUSTER
	swwcb_t *swwcb = (swwcb_t *) part->swwcb_mem;
	ind_cluster_arg.part_buffer = part->r_part_buffer;
	ind_cluster_arg.in_part = task->r_part_buffer;
	ind_cluster_arg.hist = part->r_hist;
	ind_cluster_arg.pvt_buf = part->r_pvt_buf;
	ind_cluster_arg.pvt_buf_idx = part->r_pvt_buf_idx;
	ind_cluster_arg.pvt_buf_num = part->r_pvt_buf_num;
	ind_cluster_arg.rel_flag = 0;
	ind_cluster_optimized(&ind_cluster_arg, swwcb);

	ind_cluster_arg.part_buffer = part->s_part_buffer;
	ind_cluster_arg.in_part = task->s_part_buffer;
	ind_cluster_arg.hist = part->s_hist;
	ind_cluster_arg.pvt_buf = part->s_pvt_buf;
	ind_cluster_arg.pvt_buf_idx = part->s_pvt_buf_idx;
	ind_cluster_arg.pvt_buf_num = part->s_pvt_buf_num;
	ind_cluster_arg.rel_flag = 1;
	ind_cluster_optimized(&ind_cluster_arg, swwcb);
...
void ind_lp_join(task_t * const task, join_arg_t * const join_arg) {
...
void ind_sc_join(task_t * const task, join_arg_t * const join_arg) {
	join_arg -> matched_cnt = 0;
	join_arg -> checksum = 0;
...
void ind_hr_join(task_t * const task, join_arg_t * const join_arg) {
	join_arg -> matched_cnt = 0;
	join_arg -> checksum = 0;
...
void ind_hro_join(task_t * const task, join_arg_t * const join_arg) {
	join_arg -> matched_cnt = 0;
	join_arg -> checksum = 0;
	...
	/* indink the hashtable size according to AVX512_SIZE/TUPLE_T_SIZE */
	max_hashkey >>= HR_HRO_FACTOR;
	if (max_hashkey < (1<<HR_HRO_FACTOR)) {
		max_hashkey = (1<<HR_HRO_FACTOR);
	}
...
void *ind_prj_thread_buildpart(void *param) {
	phj_ind_arg_t *phj_arg = (phj_ind_arg_t *) param;
...
#ifdef USE_NVM
	phj_arg->r_1st_pvt_buf = (ind_part_buffer_t *) new_alloc_nvm(IND_PART_BUFFER_T_SIZE*(phj_arg->r_pvt_buf_num));
	phj_arg->s_1st_pvt_buf = (ind_part_buffer_t *) new_alloc_nvm(IND_PART_BUFFER_T_SIZE*(phj_arg->s_pvt_buf_num));
#else /* USE_NVM */
#ifdef USE_HUGE
	phj_arg->r_1st_pvt_buf = (ind_part_buffer_t *) alloc_huge_dram(IND_PART_BUFFER_T_SIZE*(phj_arg->r_pvt_buf_num));
	phj_arg->s_1st_pvt_buf = (ind_part_buffer_t *) alloc_huge_dram(IND_PART_BUFFER_T_SIZE*(phj_arg->s_pvt_buf_num));
#else /* USE_HUGE */
	phj_arg->r_1st_pvt_buf = (ind_part_buffer_t *) alloc_aligned_dram(IND_PART_BUFFER_T_SIZE*(phj_arg->r_pvt_buf_num), CACHELINE_SIZE);
	phj_arg->s_1st_pvt_buf = (ind_part_buffer_t *) alloc_aligned_dram(IND_PART_BUFFER_T_SIZE*(phj_arg->s_pvt_buf_num), CACHELINE_SIZE);
#endif /* USE_HUGE */
#endif /* USE_NVM */
	memset_localize(phj_arg->r_1st_pvt_buf, IND_PART_BUFFER_T_SIZE*(phj_arg->r_pvt_buf_num));
	memset_localize(phj_arg->s_1st_pvt_buf, IND_PART_BUFFER_T_SIZE*(phj_arg->s_pvt_buf_num));
	size_t r_1st_pvt_buf_idx = 0;
	size_t s_1st_pvt_buf_idx = 0;
#if NUM_PASSES != 1
#ifdef USE_NVM
	phj_arg->r_2nd_pvt_buf = (ind_part_buffer_t *) new_alloc_nvm(IND_PART_BUFFER_T_SIZE*(phj_arg->r_pvt_buf_num));
	phj_arg->s_2nd_pvt_buf = (ind_part_buffer_t *) new_alloc_nvm(IND_PART_BUFFER_T_SIZE*(phj_arg->s_pvt_buf_num));
#else /* USE_NVM */
#ifdef USE_HUGE
	phj_arg->r_2nd_pvt_buf = (ind_part_buffer_t *) alloc_huge_dram(IND_PART_BUFFER_T_SIZE*(phj_arg->r_pvt_buf_num));
	phj_arg->s_2nd_pvt_buf = (ind_part_buffer_t *) alloc_huge_dram(IND_PART_BUFFER_T_SIZE*(phj_arg->s_pvt_buf_num));
#else /* USE_HUGE */
	phj_arg->r_2nd_pvt_buf = (ind_part_buffer_t *) alloc_aligned_dram(IND_PART_BUFFER_T_SIZE*(phj_arg->r_pvt_buf_num), CACHELINE_SIZE);
	phj_arg->s_2nd_pvt_buf = (ind_part_buffer_t *) alloc_aligned_dram(IND_PART_BUFFER_T_SIZE*(phj_arg->s_pvt_buf_num), CACHELINE_SIZE);
#endif /* USE_HUGE */
#endif /* USE_NVM */
	memset_localize(phj_arg->r_2nd_pvt_buf, IND_PART_BUFFER_T_SIZE*(phj_arg->r_pvt_buf_num));
	memset_localize(phj_arg->s_2nd_pvt_buf, IND_PART_BUFFER_T_SIZE*(phj_arg->s_pvt_buf_num));
	size_t r_2nd_pvt_buf_idx = 0;
	size_t s_2nd_pvt_buf_idx = 0;
#else /* NUM_PASSES != 1 */
#ifdef SKEW_HANDLING
#ifdef USE_NVM
	phj_arg->r_2nd_pvt_buf = (ind_part_buffer_t *) new_alloc_nvm(IND_PART_BUFFER_T_SIZE*(phj_arg->r_pvt_buf_num));
	...
	phj_arg->r_part_hist[phj_arg->_tid] = (size_t *) alloc_memory( sizeof(size_t) * FANOUT_PASS1, CACHELINE_SIZE);
	phj_arg->s_part_hist[phj_arg->_tid] = (size_t *) alloc_memory( sizeof(size_t) * FANOUT_PASS1, CACHELINE_SIZE);

	phj_arg->r_is_rear_flag[phj_arg->_tid] = (bool*) malloc(sizeof(bool) * FANOUT_PASS1);
	phj_arg->s_is_rear_flag[phj_arg->_tid] = (bool*) malloc(sizeof(bool) * FANOUT_PASS1);

#ifdef SKEW_HANDLING
	phj_arg->skew_hand_r_part_hist[phj_arg->_tid] = (size_t *) alloc_memory( sizeof(size_t) * FANOUT_PASS2, CACHELINE_SIZE);
	phj_arg->skew_hand_s_part_hist[phj_arg->_tid] = (size_t *) alloc_memory( sizeof(size_t) * FANOUT_PASS2, CACHELINE_SIZE);

	phj_arg->skew_r_is_rear_flag[phj_arg->_tid] = (bool*) malloc(sizeof(bool) * FANOUT_PASS2);
	phj_arg->skew_s_is_rear_flag[phj_arg->_tid] = (bool*) malloc(sizeof(bool) * FANOUT_PASS2);

	phj_arg->skew_hand_r_link_buffer_num[phj_arg->_tid] = (size_t *) alloc_memory( sizeof(size_t) * FANOUT_PASS1, CACHELINE_SIZE );
	phj_arg->skew_hand_s_link_buffer_num[phj_arg->_tid] = (size_t *) alloc_memory( sizeof(size_t) * FANOUT_PASS1, CACHELINE_SIZE );

	phj_arg->skew_skew_r_link_buffer_num[phj_arg->_tid] = (size_t *) alloc_memory( sizeof(size_t) * FANOUT_PASS2, CACHELINE_SIZE );
	phj_arg->skew_skew_s_link_buffer_num[phj_arg->_tid] = (size_t *) alloc_memory( sizeof(size_t) * FANOUT_PASS2, CACHELINE_SIZE );

	ind_part_buffer_t *r_buff_org = phj_arg->r_part_buffer;
	ind_part_buffer_t *s_buff_org = phj_arg->s_part_buffer;
	ind_part_buffer_t *r_rear_org = phj_arg->r_part_rear;
	ind_part_buffer_t *s_rear_org = phj_arg->s_part_rear;
#endif /* SKEW_HANDLING */
	...
	memset_localize(phj_arg->r_part_hist[phj_arg->_tid] , sizeof(size_t) * FANOUT_PASS1);
	memset_localize(phj_arg->s_part_hist[phj_arg->_tid] , sizeof(size_t) * FANOUT_PASS1);

	memset(phj_arg->r_is_rear_flag[phj_arg->_tid], 0, sizeof(bool) * FANOUT_PASS1);
	memset(phj_arg->s_is_rear_flag[phj_arg->_tid], 0, sizeof(bool) * FANOUT_PASS1);

#ifdef SKEW_HANDLING
	memset_localize(phj_arg->skew_hand_r_link_buffer_num[phj_arg->_tid], sizeof(size_t) * FANOUT_PASS1);
	memset_localize(phj_arg->skew_hand_s_link_buffer_num[phj_arg->_tid], sizeof(size_t) * FANOUT_PASS1);
#endif /* SKEW_HANDLING */
	...
	part.part_rear = phj_arg->r_part_rear;
	part.is_rear_flag = phj_arg->r_is_rear_flag[phj_arg->_tid];
	part.output_offset = phj_arg->r_part_hist[phj_arg->_tid];

	part.pvt_buf = phj_arg->r_1st_pvt_buf;
	part.pvt_buf_idx = &r_1st_pvt_buf_idx;
	part.pvt_buf_num = phj_arg->r_pvt_buf_num;

#ifdef SKEW_HANDLING
	part.skew_hand_link_buffer_num = phj_arg->skew_hand_r_link_buffer_num[phj_arg->_tid];
#endif /* SKEW_HANDLING */

	part.rel_flag = 0;

#ifdef USE_SWWCB_OPTIMIZED_PART
	ind_parallel_partition_optimized_first(&part, swwcb);
#else /* USE_SWWCB_OPTIMIZED_PART */
	ind_parallel_partition_first(&part);
#endif /* USE_SWWCB_OPTIMIZED_PART */

	/**************** PARTITIONING S ****************/
	part.rel = phj_arg->s_rel;
	part.part_buffer = phj_arg->s_part_buffer;
	part.part_rear = phj_arg->s_part_rear;
	part.is_rear_flag = phj_arg->s_is_rear_flag[phj_arg->_tid];
	part.output_offset = phj_arg->s_part_hist[phj_arg->_tid];

	part.pvt_buf = phj_arg->s_1st_pvt_buf;
	part.pvt_buf_idx = &s_1st_pvt_buf_idx;
	part.pvt_buf_num = phj_arg->s_pvt_buf_num;

	part.rel_flag = 1;

#ifdef SKEW_HANDLING
	part.skew_hand_link_buffer_num = phj_arg->skew_hand_s_link_buffer_num[phj_arg->_tid];
#endif /* SKEW_HANDLING */
	...
	if (phj_arg->_tid == 0) {
		...
		ind_part_buffer_t *tmp_head, *tmp_rear;

#ifdef SKEW_HANDLING
		size_t skew_hand_r_link_buffer_num;
		size_t skew_hand_s_link_buffer_num;
#endif /* SKEW_HANDLING */

		for (size_t idx = 0; idx < FANOUT_PASS1; idx ++) {
			r_tup_num = phj_arg->r_part_hist[0][idx];
			s_tup_num = phj_arg->s_part_hist[0][idx];

#ifdef SKEW_HANDLING
			/* initially, each single partition has one buffer, even it could be empty */
			skew_hand_r_link_buffer_num = phj_arg->skew_hand_r_link_buffer_num[0][idx]+1;
			skew_hand_s_link_buffer_num = phj_arg->skew_hand_s_link_buffer_num[0][idx]+1;
#endif /* SKEW_HANDLING */

			for (int jdx = 1; jdx < BUILDPART_THREAD_NUM; jdx ++) {
				r_tup_num += phj_arg->r_part_hist[jdx][idx];
				s_tup_num += phj_arg->s_part_hist[jdx][idx];

				if (phj_arg->r_is_rear_flag[jdx-1][idx]) {
					tmp_rear = phj_arg->r_part_rear + (jdx-1)*(FANOUT_PASS1+FANOUT_PASS1*FANOUT_PASS2) + idx;
				} else {
					tmp_rear = phj_arg->r_part_buffer + (jdx-1)*(FANOUT_PASS1+FANOUT_PASS1*FANOUT_PASS2) + idx;
				}
				tmp_head = phj_arg->r_part_buffer + jdx*(FANOUT_PASS1+FANOUT_PASS1*FANOUT_PASS2) + idx;
				tmp_rear->next = tmp_head;

				if (phj_arg->s_is_rear_flag[jdx-1][idx]) {
					tmp_rear = phj_arg->s_part_rear + (jdx-1)*(FANOUT_PASS1+FANOUT_PASS1*FANOUT_PASS2) + idx;
				} else {
					tmp_rear = phj_arg->s_part_buffer + (jdx-1)*(FANOUT_PASS1+FANOUT_PASS1*FANOUT_PASS2) + idx;
				}

				tmp_head = phj_arg->s_part_buffer + jdx*(FANOUT_PASS1+FANOUT_PASS1*FANOUT_PASS2) + idx;

				tmp_rear->next = tmp_head;

#ifdef SKEW_HANDLING
				/* initially, each single partition has one buffer, even it could be empty */
				skew_hand_r_link_buffer_num += phj_arg->skew_hand_r_link_buffer_num[jdx][idx]+1;
				skew_hand_s_link_buffer_num += phj_arg->skew_hand_s_link_buffer_num[jdx][idx]+1;
#endif /* SKEW_HANDLING */
			}

#ifdef SKEW_HANDLING
			if (r_tup_num > THRESHOLD_0 || s_tup_num > THRESHOLD_0) {
				task = task_queue_get_slot(skew_queue);
				task->r_tuple_num = r_tup_num;
				task->r_part_buffer = phj_arg->r_part_buffer + idx;
				task->s_tuple_num = s_tup_num;
				task->s_part_buffer = phj_arg->s_part_buffer + idx;
				task->r_link_buffer_num = skew_hand_r_link_buffer_num;
				task->s_link_buffer_num = skew_hand_s_link_buffer_num;
			...
			if (r_tup_num > 0 || s_tup_num > 0) {
				...
				max_build_side_tup_num = max_build_side_tup_num >= r_tup_num ? 
					max_build_side_tup_num : r_tup_num;
			}
	...
	phj_arg->r_part_rear += FANOUT_PASS1;
	phj_arg->s_part_rear += FANOUT_PASS1;
	...
	size_t ind_serial_part_count = 0;

	ind_serial_part_t ind_serial_part;
	...
	ind_serial_part.max_build_side_tup_num = &max_build_side_tup_num;

	ind_serial_part.r_pvt_buf = phj_arg->r_2nd_pvt_buf;
	ind_serial_part.r_pvt_buf_idx = &r_2nd_pvt_buf_idx;
	ind_serial_part.r_pvt_buf_num = phj_arg->r_pvt_buf_num;

	ind_serial_part.s_pvt_buf = phj_arg->s_2nd_pvt_buf;
	ind_serial_part.s_pvt_buf_idx = &s_2nd_pvt_buf_idx;
	ind_serial_part.s_pvt_buf_num = phj_arg->s_pvt_buf_num;

	while ( ( task = task_queue_get_count_atomic(part_queue, &ind_serial_part_count) ) ) {
		ind_serial_part.r_part_buffer = phj_arg->r_part_buffer+FANOUT_PASS2*(ind_serial_part_count-1);
		ind_serial_part.s_part_buffer = phj_arg->s_part_buffer+FANOUT_PASS2*(ind_serial_part_count-1);
		ind_serial_partition(task, phj_arg->join_queue, &ind_serial_part);
	}
	...
	phj_arg->r_part_rear += FANOUT_PASS2*part_queue_num;
	phj_arg->s_part_rear += FANOUT_PASS2*part_queue_num;
#endif /* NUM_PASSES == 2 */
	...
	while(1) {
		...
		memset_localize(phj_arg->skew_hand_r_part_hist[phj_arg->_tid], sizeof(size_t) * FANOUT_PASS2);
		memset_localize(phj_arg->skew_hand_s_part_hist[phj_arg->_tid], sizeof(size_t) * FANOUT_PASS2);
		memset(phj_arg->skew_r_is_rear_flag[phj_arg->_tid], 0, sizeof(bool) * FANOUT_PASS2);
		memset(phj_arg->skew_s_is_rear_flag[phj_arg->_tid], 0, sizeof(bool) * FANOUT_PASS2);
		memset_localize(phj_arg->skew_skew_r_link_buffer_num[phj_arg->_tid], sizeof(size_t) * FANOUT_PASS2);
		memset_localize(phj_arg->skew_skew_s_link_buffer_num[phj_arg->_tid], sizeof(size_t) * FANOUT_PASS2);
		...
		/**************** PARTITIONING R ****************/
		skew_part.part_buffer = phj_arg->r_part_buffer + FANOUT_PASS2*skew_queue->count;
		skew_part.part_rear = phj_arg->r_part_rear + FANOUT_PASS2*skew_queue->count;
		skew_part.in_part = (*skewtask)->r_part_buffer;
		skew_part.output_offset = phj_arg->skew_hand_r_part_hist[phj_arg->_tid];
		...
		skew_part.is_rear_flag = phj_arg->skew_r_is_rear_flag[phj_arg->_tid];
		skew_part.pvt_buf = phj_arg->r_2nd_pvt_buf;
		skew_part.pvt_buf_idx = &r_2nd_pvt_buf_idx;
		skew_part.pvt_buf_num = phj_arg->r_pvt_buf_num;

		skew_part.skew_skew_link_buffer_num = phj_arg->skew_skew_r_link_buffer_num[phj_arg->_tid];

#ifdef USE_SWWCB_OPTIMIZED_PART
		ind_parallel_partition_optimized_following(&skew_part, skew_hand_swwcb);
#else /* USE_SWWCB_OPTIMIZED_PART */
		ind_parallel_partition_following(&skew_part);
#endif /* USE_SWWCB_OPTIMIZED_PART */

		/**************** PARTITIONING S ****************/
		skew_part.part_buffer = phj_arg->s_part_buffer + FANOUT_PASS2*skew_queue->count;
		skew_part.part_rear = phj_arg->s_part_rear + FANOUT_PASS2*skew_queue->count;
		skew_part.in_part = (*skewtask)->s_part_buffer;
		skew_part.output_offset = phj_arg->skew_hand_s_part_hist[phj_arg->_tid];
		...
		skew_part.is_rear_flag = phj_arg->skew_s_is_rear_flag[phj_arg->_tid];
		skew_part.pvt_buf = phj_arg->s_2nd_pvt_buf;
		skew_part.pvt_buf_idx = &s_2nd_pvt_buf_idx;
		skew_part.pvt_buf_num = phj_arg->s_pvt_buf_num;

		skew_part.skew_skew_link_buffer_num = phj_arg->skew_skew_s_link_buffer_num[phj_arg->_tid];
		...
		if (phj_arg -> _tid == 0) {
			...
			// size_t skew_skew_r_link_buffer_num;
			size_t skew_skew_s_link_buffer_num;

			for (size_t idx = 0; idx < FANOUT_PASS2; idx ++) {

				r_tup_num = phj_arg->skew_hand_r_part_hist[0][idx];
				s_tup_num = phj_arg->skew_hand_s_part_hist[0][idx];

				/* initially, each single partition has one buffer, even it could be empty */
				// skew_skew_r_link_buffer_num = phj_arg->skew_skew_r_link_buffer_num[0][idx]+1;
				skew_skew_s_link_buffer_num = phj_arg->skew_skew_s_link_buffer_num[0][idx]+1;

				for (int jdx = 1; jdx < BUILDPART_THREAD_NUM; jdx ++) {
					r_tup_num += phj_arg->skew_hand_r_part_hist[jdx][idx];
					s_tup_num += phj_arg->skew_hand_s_part_hist[jdx][idx];

					if (phj_arg->skew_r_is_rear_flag[jdx-1][idx]) {
						tmp_rear = r_rear_org + (jdx-1)*(FANOUT_PASS1+FANOUT_PASS1*FANOUT_PASS2) + 
							FANOUT_PASS1 + FANOUT_PASS2*part_queue_num + FANOUT_PASS2*skew_queue->count + idx ;
					} else {
						tmp_rear = r_buff_org + (jdx-1)*(FANOUT_PASS1+FANOUT_PASS1*FANOUT_PASS2) + 
							FANOUT_PASS1 + FANOUT_PASS2*part_queue_num + FANOUT_PASS2*skew_queue->count + idx;
					}

					tmp_head = r_buff_org + jdx*(FANOUT_PASS1+FANOUT_PASS1*FANOUT_PASS2) + 
						FANOUT_PASS1 + FANOUT_PASS2*part_queue_num + FANOUT_PASS2*skew_queue->count + idx;
					
					tmp_rear->next = tmp_head;

					if (phj_arg->skew_s_is_rear_flag[jdx-1][idx]) {
						tmp_rear = s_rear_org + (jdx-1)*(FANOUT_PASS1+FANOUT_PASS1*FANOUT_PASS2) + 
							FANOUT_PASS1 + FANOUT_PASS2*part_queue_num + FANOUT_PASS2*skew_queue->count + idx;
					} else {
						tmp_rear = s_buff_org + (jdx-1)*(FANOUT_PASS1+FANOUT_PASS1*FANOUT_PASS2) + 
							FANOUT_PASS1 + FANOUT_PASS2*part_queue_num + FANOUT_PASS2*skew_queue->count + idx;
					}

					tmp_head = s_buff_org + jdx*(FANOUT_PASS1+FANOUT_PASS1*FANOUT_PASS2) + 
						FANOUT_PASS1 + FANOUT_PASS2*part_queue_num + FANOUT_PASS2*skew_queue->count + idx;

					tmp_rear->next = tmp_head;

					/* initially, each single partition has one buffer, even it could be empty */
					// skew_skew_r_link_buffer_num += phj_arg->skew_skew_r_link_buffer_num[jdx][idx]+1;
					skew_skew_s_link_buffer_num += phj_arg->skew_skew_s_link_buffer_num[jdx][idx]+1;
				}

				max_build_side_tup_num = max_build_side_tup_num > r_tup_num ?
					max_build_side_tup_num : r_tup_num;

				if (r_tup_num > THRESHOLD_1 && s_tup_num > THRESHOLD_1) {
					for (int jdx = 0; jdx < BUILDPART_THREAD_NUM; jdx ++) {
						skew_hand_task = task_queue_get_slot(part_queue);
						skew_hand_task->r_tuple_num = r_tup_num;
						skew_hand_task->r_part_buffer = phj_arg->r_part_buffer + FANOUT_PASS2*skew_queue->count + idx;

						skew_hand_task->s_part_buffer = phj_arg->s_part_buffer + FANOUT_PASS2*skew_queue->count + idx;

						skew_hand_task->skew_skew_flag = 1;
						skew_hand_task->skew_skew_s_start_idx = (jdx == 0) ? 0 :
							jdx * ( skew_skew_s_link_buffer_num / BUILDPART_THREAD_NUM );
						skew_hand_task->skew_skew_s_buffer_num = (jdx < BUILDPART_THREAD_NUM-1) ?
							skew_skew_s_link_buffer_num / BUILDPART_THREAD_NUM :
							skew_skew_s_link_buffer_num - jdx * (skew_skew_s_link_buffer_num / BUILDPART_THREAD_NUM);

						task_queue_add(part_queue, skew_hand_task);
					}
				} else if (r_tup_num > 0 && s_tup_num > 0) {
					skew_hand_task = task_queue_get_slot(phj_arg->join_queue);
					skew_hand_task->r_tuple_num = r_tup_num;
					skew_hand_task->r_part_buffer = phj_arg->r_part_buffer + FANOUT_PASS2*skew_queue->count + idx;
					skew_hand_task->s_tuple_num = s_tup_num;
					skew_hand_task->s_part_buffer = phj_arg->s_part_buffer + FANOUT_PASS2*skew_queue->count + idx;
...
#ifdef SKEW_HANDLING
	dealloc_memory(phj_arg->skew_hand_r_part_hist[phj_arg->_tid], sizeof(size_t)*FANOUT_PASS2);
	dealloc_memory(phj_arg->skew_hand_s_part_hist[phj_arg->_tid], sizeof(size_t)*FANOUT_PASS2);

	free(phj_arg->skew_r_is_rear_flag[phj_arg->_tid]);
	free(phj_arg->skew_s_is_rear_flag[phj_arg->_tid]);

	dealloc_memory(phj_arg->skew_hand_r_link_buffer_num[phj_arg->_tid], sizeof(size_t)*FANOUT_PASS1);
	dealloc_memory(phj_arg->skew_hand_s_link_buffer_num[phj_arg->_tid], sizeof(size_t)*FANOUT_PASS1);

	dealloc_memory(phj_arg->skew_skew_r_link_buffer_num[phj_arg->_tid], sizeof(size_t)*FANOUT_PASS2);
	dealloc_memory(phj_arg->skew_skew_s_link_buffer_num[phj_arg->_tid], sizeof(size_t)*FANOUT_PASS2);
#else /* SKEW_HANDLING */
	dealloc_memory(phj_arg->r_part_hist[phj_arg->_tid], sizeof(size_t) * FANOUT_PASS1);
	dealloc_memory(phj_arg->s_part_hist[phj_arg->_tid], sizeof(size_t) * FANOUT_PASS1);
#endif /* SKEW_HANDLING */
	
	free(phj_arg->r_is_rear_flag[phj_arg->_tid]);
	free(phj_arg->s_is_rear_flag[phj_arg->_tid]);

	return NULL;
}


void *ind_prj_thread_probejoin(void *param) {
	phj_ind_arg_t *phj_arg = (phj_ind_arg_t *) param;
	join_arg_t join_arg;
...
void ind_join_init_run(const datameta_t datameta, timekeeper_t * const timekeeper, 
	...
	r_shr_free_buffers_num_pass_1 += FANOUT_PASS1 * BUILDPART_THREAD_NUM;

	r_shr_free_buffers_num_pass_2 = (size_t) ceil( (double) r_relation.tuple_num / PART_BUFFER_T_CAPACITY);
	r_shr_free_buffers_num_pass_2 += FANOUT_PASS1 * FANOUT_PASS2 * BUILDPART_THREAD_NUM;

	s_shr_free_buffers_num_pass_1 = (size_t) ceil( (double) s_relation.tuple_num / PART_BUFFER_T_CAPACITY);
	s_shr_free_buffers_num_pass_1 += FANOUT_PASS1 * BUILDPART_THREAD_NUM;

	s_shr_free_buffers_num_pass_2 = (size_t) ceil( (double) s_relation.tuple_num / PART_BUFFER_T_CAPACITY);
	s_shr_free_buffers_num_pass_2 += FANOUT_PASS1 * FANOUT_PASS2 * BUILDPART_THREAD_NUM;

	size_t r_shr_free_buffers_size_pass_1 = IND_PART_BUFFER_T_SIZE*r_shr_free_buffers_num_pass_1;
	size_t s_shr_free_buffers_size_pass_1 = IND_PART_BUFFER_T_SIZE*s_shr_free_buffers_num_pass_1;
#ifdef RUN_BILLION
	r_shr_free_buffers_size_pass_1 *= RUN_BILLION_SHRINKING_FACTOR;
	s_shr_free_buffers_size_pass_1 *= RUN_BILLION_SHRINKING_FACTOR;
#endif /* RUN_BILLION */

#if NUM_PASSES != 1
	size_t r_shr_free_buffers_size_pass_2 = IND_PART_BUFFER_T_SIZE*r_shr_free_buffers_num_pass_2;
	size_t s_shr_free_buffers_size_pass_2 = IND_PART_BUFFER_T_SIZE*s_shr_free_buffers_num_pass_2;
#ifdef RUN_BILLION
	r_shr_free_buffers_size_pass_2 *= RUN_BILLION_SHRINKING_FACTOR;
	s_shr_free_buffers_size_pass_2 *= RUN_BILLION_SHRINKING_FACTOR;
#endif /* RUN_BILLION */
#else /* NUM_PASSES != 1 */
#ifdef SKEW_HANDLING
	size_t r_shr_free_buffers_size_pass_2 = IND_PART_BUFFER_T_SIZE*r_shr_free_buffers_num_pass_2;
...
#ifdef USE_NVM
	r_shr_free_buffers_pass_1 = (ind_part_buffer_t *) new_alloc_nvm(r_shr_free_buffers_size_pass_1);
	s_shr_free_buffers_pass_1 = (ind_part_buffer_t *) new_alloc_nvm(s_shr_free_buffers_size_pass_1);
#else /* USE_NVM */
#ifdef USE_HUGE
	r_shr_free_buffers_pass_1 = (ind_part_buffer_t *) alloc_huge_dram(r_shr_free_buffers_size_pass_1);
	s_shr_free_buffers_pass_1 = (ind_part_buffer_t *) alloc_huge_dram(s_shr_free_buffers_size_pass_1);
#else /* USE_HUGE */
	r_shr_free_buffers_pass_1 = (ind_part_buffer_t *) alloc_aligned_dram(r_shr_free_buffers_size_pass_1, CACHELINE_SIZE);
	s_shr_free_buffers_pass_1 = (ind_part_buffer_t *) alloc_aligned_dram(s_shr_free_buffers_size_pass_1, CACHELINE_SIZE);
#endif /* USE_HUGE */
#endif /* USE_NVM */
	parallel_memset_localize(r_shr_free_buffers_pass_1, r_shr_free_buffers_size_pass_1);
	parallel_memset_localize(s_shr_free_buffers_pass_1, s_shr_free_buffers_size_pass_1);

#if NUM_PASSES != 1
#ifdef USE_NVM	
	r_shr_free_buffers_pass_2 = (ind_part_buffer_t *) new_alloc_nvm(r_shr_free_buffers_size_pass_2);
	s_shr_free_buffers_pass_2 = (ind_part_buffer_t *) new_alloc_nvm(s_shr_free_buffers_size_pass_2);
#else /* USE_NVM */
#ifdef USE_HUGE
	r_shr_free_buffers_pass_2 = (ind_part_buffer_t *) alloc_huge_dram(r_shr_free_buffers_size_pass_2);
	s_shr_free_buffers_pass_2 = (ind_part_buffer_t *) alloc_huge_dram(s_shr_free_buffers_size_pass_2);
#else /* USE_HUGE */
	r_shr_free_buffers_pass_2 = (ind_part_buffer_t *) alloc_aligned_dram(r_shr_free_buffers_size_pass_2, CACHELINE_SIZE);
	s_shr_free_buffers_pass_2 = (ind_part_buffer_t *) alloc_aligned_dram(s_shr_free_buffers_size_pass_2, CACHELINE_SIZE);
#endif /* USE_HUGE */
#endif /* USE_NVM */
	parallel_memset_localize(r_shr_free_buffers_pass_2, r_shr_free_buffers_size_pass_2);
	parallel_memset_localize(s_shr_free_buffers_pass_2, s_shr_free_buffers_size_pass_2);
#else /* NUM_PASSES != 1 */
#ifdef SKEW_HANDLING
#ifdef USE_NVM	
	r_shr_free_buffers_pass_2 = (ind_part_buffer_t *) new_alloc_nvm(r_shr_free_buffers_size_pass_2);
	...
	ind_part_buffer_t *r_part_buffer, *s_part_buffer, *r_part_rear, *s_part_rear;
#ifdef USE_NVM
	r_part_buffer = (ind_part_buffer_t *) new_alloc_nvm(IND_PART_BUFFER_T_SIZE*
		(FANOUT_PASS1+FANOUT_PASS1*FANOUT_PASS2)*BUILDPART_THREAD_NUM);
	s_part_buffer = (ind_part_buffer_t *) new_alloc_nvm(IND_PART_BUFFER_T_SIZE*
		(FANOUT_PASS1+FANOUT_PASS1*FANOUT_PASS2)*BUILDPART_THREAD_NUM);
	r_part_rear = (ind_part_buffer_t *) new_alloc_nvm(IND_PART_BUFFER_T_SIZE*
		(FANOUT_PASS1+FANOUT_PASS1*FANOUT_PASS2)*BUILDPART_THREAD_NUM);
	s_part_rear = (ind_part_buffer_t *) new_alloc_nvm(IND_PART_BUFFER_T_SIZE*
		(FANOUT_PASS1+FANOUT_PASS1*FANOUT_PASS2)*BUILDPART_THREAD_NUM);
#else /* USE_NVM */
#ifdef USE_HUGE
	r_part_buffer = (ind_part_buffer_t *) alloc_huge_dram(IND_PART_BUFFER_T_SIZE*
		(FANOUT_PASS1+FANOUT_PASS1*FANOUT_PASS2)*BUILDPART_THREAD_NUM);
	s_part_buffer = (ind_part_buffer_t *) alloc_huge_dram(IND_PART_BUFFER_T_SIZE*
		(FANOUT_PASS1+FANOUT_PASS1*FANOUT_PASS2)*BUILDPART_THREAD_NUM);
	r_part_rear = (ind_part_buffer_t *) alloc_huge_dram(IND_PART_BUFFER_T_SIZE*
		(FANOUT_PASS1+FANOUT_PASS1*FANOUT_PASS2)*BUILDPART_THREAD_NUM);
	s_part_rear = (ind_part_buffer_t *) alloc_huge_dram(IND_PART_BUFFER_T_SIZE*
		(FANOUT_PASS1+FANOUT_PASS1*FANOUT_PASS2)*BUILDPART_THREAD_NUM);
#else /* USE_HUGE */
	r_part_buffer = (ind_part_buffer_t *) alloc_aligned_dram(IND_PART_BUFFER_T_SIZE*
		(FANOUT_PASS1+FANOUT_PASS1*FANOUT_PASS2)*BUILDPART_THREAD_NUM, CACHELINE_SIZE);
	s_part_buffer = (ind_part_buffer_t *) alloc_aligned_dram(IND_PART_BUFFER_T_SIZE*
		(FANOUT_PASS1+FANOUT_PASS1*FANOUT_PASS2)*BUILDPART_THREAD_NUM, CACHELINE_SIZE);
	r_part_rear = (ind_part_buffer_t *) alloc_aligned_dram(IND_PART_BUFFER_T_SIZE*
		(FANOUT_PASS1+FANOUT_PASS1*FANOUT_PASS2)*BUILDPART_THREAD_NUM, CACHELINE_SIZE);
	s_part_rear = (ind_part_buffer_t *) alloc_aligned_dram(IND_PART_BUFFER_T_SIZE*
		(FANOUT_PASS1+FANOUT_PASS1*FANOUT_PASS2)*BUILDPART_THREAD_NUM, CACHELINE_SIZE);	
#endif /* USE_HUGE */
#endif /* USE_NVM */
	parallel_memset_localize(r_part_buffer, IND_PART_BUFFER_T_SIZE*
		(FANOUT_PASS1+FANOUT_PASS1*FANOUT_PASS2)*BUILDPART_THREAD_NUM);
	parallel_memset_localize(s_part_buffer, IND_PART_BUFFER_T_SIZE*
		(FANOUT_PASS1+FANOUT_PASS1*FANOUT_PASS2)*BUILDPART_THREAD_NUM);
	parallel_memset_localize(r_part_rear, IND_PART_BUFFER_T_SIZE*
		(FANOUT_PASS1+FANOUT_PASS1*FANOUT_PASS2)*BUILDPART_THREAD_NUM);
	parallel_memset_localize(s_part_rear, IND_PART_BUFFER_T_SIZE*
		(FANOUT_PASS1+FANOUT_PASS1*FANOUT_PASS2)*BUILDPART_THREAD_NUM);


	/**************** CREATE PART_TUP_NUM FOR PARTITIONING ****************/
	size_t **r_part_hist = (size_t **) alloc_memory(sizeof(size_t *) * BUILDPART_THREAD_NUM, CACHELINE_SIZE);
	size_t **s_part_hist = (size_t **) alloc_memory(sizeof(size_t *) * BUILDPART_THREAD_NUM, CACHELINE_SIZE);
	bool **r_is_rear_flag = (bool **) malloc(sizeof(bool *) * BUILDPART_THREAD_NUM);
	bool **s_is_rear_flag = (bool **) malloc(sizeof(bool *) * BUILDPART_THREAD_NUM);
	...
	size_t **skew_hand_r_part_hist = (size_t **) alloc_memory( sizeof(size_t *) * BUILDPART_THREAD_NUM, CACHELINE_SIZE);
	size_t **skew_hand_s_part_hist = (size_t **) alloc_memory( sizeof(size_t *) * BUILDPART_THREAD_NUM, CACHELINE_SIZE);

	bool **skew_r_is_rear_flag = (bool **) malloc(sizeof(bool *) * BUILDPART_THREAD_NUM);
	bool **skew_s_is_rear_flag = (bool **) malloc(sizeof(bool *) * BUILDPART_THREAD_NUM);

	size_t **skew_hand_r_link_buffer_num = (size_t **) alloc_memory( sizeof(size_t *) * BUILDPART_THREAD_NUM, CACHELINE_SIZE );
	size_t **skew_hand_s_link_buffer_num = (size_t **) alloc_memory( sizeof(size_t *) * BUILDPART_THREAD_NUM, CACHELINE_SIZE );

	size_t **skew_skew_r_link_buffer_num = (size_t **) alloc_memory( sizeof(size_t *) * BUILDPART_THREAD_NUM, CACHELINE_SIZE );
	size_t **skew_skew_s_link_buffer_num = (size_t **) alloc_memory( sizeof(size_t *) * BUILDPART_THREAD_NUM, CACHELINE_SIZE );
#endif /* SKEW_HANDLING */
	...
	phj_ind_arg_t args[max_thread_num];
...
#ifdef SYNCSTATS
	synctimer_phj_ind_t *localtimer = (synctimer_phj_ind_t *) malloc( max_thread_num * sizeof(synctimer_phj_ind_t) );
	synctimer_phj_ind_t *globaltimer = (synctimer_phj_ind_t *) malloc( sizeof(synctimer_phj_ind_t) );
	memset(localtimer, 0, max_thread_num * sizeof(synctimer_phj_ind_t));
	memset(globaltimer, 0, sizeof(synctimer_phj_ind_t));
#endif /* SYNCSTATS */
	...
	for (int idx = 0; idx < BUILDPART_THREAD_NUM; idx ++) {
		...
		(args+idx)->r_part_buffer = r_part_buffer + 
			idx*(FANOUT_PASS1+FANOUT_PASS1*FANOUT_PASS2);
		(args+idx)->r_part_rear = r_part_rear + 
			idx*(FANOUT_PASS1+FANOUT_PASS1*FANOUT_PASS2);
		(args+idx)->r_pvt_buf_num = (idx < BUILDPART_THREAD_NUM) ? 
			r_relation.tuple_num/(PART_BUFFER_T_CAPACITY)/BUILDPART_THREAD_NUM
			: r_relation.tuple_num/(PART_BUFFER_T_CAPACITY) - 
			idx * r_relation.tuple_num/(PART_BUFFER_T_CAPACITY)/BUILDPART_THREAD_NUM;
		...
		(args+idx)->s_part_buffer = s_part_buffer + 
			idx*(FANOUT_PASS1+FANOUT_PASS1*FANOUT_PASS2);
		(args+idx)->s_part_rear = s_part_rear + 
			idx*(FANOUT_PASS1+FANOUT_PASS1*FANOUT_PASS2);
		(args+idx)->s_pvt_buf_num = (idx < BUILDPART_THREAD_NUM) ? 
			s_relation.tuple_num/(PART_BUFFER_T_CAPACITY)/BUILDPART_THREAD_NUM
			: s_relation.tuple_num/(PART_BUFFER_T_CAPACITY) - 
			idx * s_relation.tuple_num/(PART_BUFFER_T_CAPACITY)/BUILDPART_THREAD_NUM;
		...
		(args+idx)->r_is_rear_flag = r_is_rear_flag;
		(args+idx)->s_is_rear_flag = s_is_rear_flag;
		...
		(args+idx)->skew_r_is_rear_flag = skew_r_is_rear_flag;
		(args+idx)->skew_s_is_rear_flag = skew_s_is_rear_flag;

		(args+idx)->skew_hand_r_link_buffer_num = skew_hand_r_link_buffer_num;
		(args+idx)->skew_hand_s_link_buffer_num = skew_hand_s_link_buffer_num;

		(args+idx)->skew_skew_r_link_buffer_num = skew_skew_r_link_buffer_num;
		(args+idx)->skew_skew_s_link_buffer_num = skew_skew_s_link_buffer_num;
#endif /* SKEW_HANDLING */
		...
		rv = pthread_create(&threadpool[idx], &attr, ind_prj_thread_buildpart, args+idx);
		if (rv){
	...
	for (int idx = 0; idx < PROBEJOIN_THREAD_NUM; idx ++) {
		...
		rv = pthread_create(&threadpool[idx], &attr, ind_prj_thread_probejoin, args+idx);
		if (rv){
			printf("ERROR; return code from pthread_create() is %d\n", rv);
			exit(EXIT_FAILURE);
		}
	...
	dealloc_memory(skew_hand_r_part_hist, sizeof(size_t *) * BUILDPART_THREAD_NUM);
	dealloc_memory(skew_hand_s_part_hist, sizeof(size_t *) * BUILDPART_THREAD_NUM);

	free(skew_r_is_rear_flag);
	free(skew_s_is_rear_flag);

	dealloc_memory(skew_hand_r_link_buffer_num, sizeof(size_t *) * BUILDPART_THREAD_NUM);
	dealloc_memory(skew_hand_s_link_buffer_num, sizeof(size_t *) * BUILDPART_THREAD_NUM);

	dealloc_memory(skew_skew_r_link_buffer_num, sizeof(size_t *) * BUILDPART_THREAD_NUM);
	dealloc_memory(skew_skew_s_link_buffer_num, sizeof(size_t *) * BUILDPART_THREAD_NUM);
#endif /* SKEW_HANDLING */

	dealloc_memory(r_part_hist, sizeof(size_t *) * BUILDPART_THREAD_NUM);
	dealloc_memory(s_part_hist, sizeof(size_t *) * BUILDPART_THREAD_NUM);

	free(r_is_rear_flag);
	free(s_is_rear_flag);

	dealloc_memory(r_shr_free_buffers_pass_1, r_shr_free_buffers_size_pass_1);
	dealloc_memory(s_shr_free_buffers_pass_1, s_shr_free_buffers_size_pass_1);
#if NUM_PASSES != 1
	dealloc_memory(r_shr_free_buffers_pass_2, r_shr_free_buffers_size_pass_1);
	dealloc_memory(s_shr_free_buffers_pass_2, s_shr_free_buffers_size_pass_1);
#else /* NUM_PASSES != 1 */
#ifdef SKEW_HANDLING
	dealloc_memory(r_shr_free_buffers_pass_2, r_shr_free_buffers_size_pass_1);
	dealloc_memory(s_shr_free_buffers_pass_2, s_shr_free_buffers_size_pass_1);
	...
	for (int idx = 0; idx < BUILDPART_THREAD_NUM; idx ++) {
		dealloc_memory((args+idx)->r_1st_pvt_buf, IND_PART_BUFFER_T_SIZE*((args+idx)->r_pvt_buf_num));
		dealloc_memory((args+idx)->s_1st_pvt_buf, IND_PART_BUFFER_T_SIZE*((args+idx)->s_pvt_buf_num));
#if NUM_PASSES != 1
		dealloc_memory((args+idx)->r_2nd_pvt_buf, IND_PART_BUFFER_T_SIZE*((args+idx)->r_pvt_buf_num));
		dealloc_memory((args+idx)->s_2nd_pvt_buf, IND_PART_BUFFER_T_SIZE*((args+idx)->s_pvt_buf_num));
#else /* NUM_PASSES != 1 */
#ifdef SKEW_HANDLING
		dealloc_memory((args+idx)->r_2nd_pvt_buf, IND_PART_BUFFER_T_SIZE*((args+idx)->r_pvt_buf_num));
		dealloc_memory((args+idx)->s_2nd_pvt_buf, IND_PART_BUFFER_T_SIZE*((args+idx)->s_pvt_buf_num));
	...
	dealloc_memory(r_part_buffer, IND_PART_BUFFER_T_SIZE*
		(FANOUT_PASS1+FANOUT_PASS1*FANOUT_PASS2)*BUILDPART_THREAD_NUM);
	dealloc_memory(s_part_buffer, IND_PART_BUFFER_T_SIZE*
		(FANOUT_PASS1+FANOUT_PASS1*FANOUT_PASS2)*BUILDPART_THREAD_NUM);
	dealloc_memory(r_part_rear, IND_PART_BUFFER_T_SIZE*
		(FANOUT_PASS1+FANOUT_PASS1*FANOUT_PASS2)*BUILDPART_THREAD_NUM);
	dealloc_memory(s_part_rear, IND_PART_BUFFER_T_SIZE*
		(FANOUT_PASS1+FANOUT_PASS1*FANOUT_PASS2)*BUILDPART_THREAD_NUM);
...
void phj_ind_lp(const datameta_t datameta, timekeeper_t * const timekeeper) {
	ind_join_init_run(datameta, timekeeper, ind_lp_join);
}

void phj_ind_sc(const datameta_t datameta, timekeeper_t * const timekeeper) {
	ind_join_init_run(datameta, timekeeper, ind_sc_join);
}

void phj_ind_hr(const datameta_t datameta, timekeeper_t * const timekeeper) {
	ind_join_init_run(datameta, timekeeper, ind_hr_join);
}

#if TUPLE_T_SIZE <= AVX512_SIZE
void phj_ind_hro(const datameta_t datameta, timekeeper_t * const timekeeper) {
	ind_join_init_run(datameta, timekeeper, ind_hro_join);
}
#endif /* TUPLE_T_SIZE <= AVX512_SIZE */