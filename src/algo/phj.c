...
extern int cur_t;
typedef void (*joinfunction_t)(const relation_t * const, const relation_t * const, relation_t * const, join_arg_t * const);


/** holds the arguments passed to each thread */
typedef struct phj_arg_t {
	int _tid;
...
void parallel_radix_partition(part_t * const part, const size_t padding_num) {
	int rv;
...
#ifdef USE_SWWCB_OPTIMIZED_PART
void parallel_radix_partition_optimized(part_t * const part, swwcb_t * const swwcb, 
...
void radix_cluster(relation_t out_rel, relation_t in_rel, size_t *hist, 
...
void radix_cluster_optimized(relation_t out_rel, relation_t in_rel, size_t *hist, const size_t fanout, 
...
void serial_radix_partition(task_t * const task, task_queue_t * join_queue, const size_t fanout, const key_t_ bitskip, 
	...
	radix_cluster_optimized(task->r_tmp, task->r_rel, r_hist, fanout, bitskip, swwcb, padding_num, task->r_global_offset);
	radix_cluster_optimized(task->s_tmp, task->s_rel, s_hist, fanout, bitskip, swwcb, padding_num, task->s_global_offset);
#else /* USE_SWWCB_OPTIMIZED_RADIX_CLUSTER_ON_NVM */
	radix_cluster(task->r_tmp, task->r_rel, r_hist, fanout, bitskip, padding_num);
	radix_cluster(task->s_tmp, task->s_rel, s_hist, fanout, bitskip, padding_num);
...
void bc_join(const relation_t * const r_rel, const relation_t * const s_rel, relation_t * const r_tmp, join_arg_t * const join_arg) {
...
void lp_join(const relation_t * const r_rel, const relation_t * const s_rel, relation_t * const r_tmp, join_arg_t * const join_arg) {
	join_arg -> matched_cnt = 0;
	join_arg -> checksum = 0;
...
void sc_join(const relation_t * const r_rel, const relation_t * const s_rel, relation_t * const r_tmp, join_arg_t * const join_arg) {
	join_arg -> matched_cnt = 0;
	join_arg -> checksum = 0;
...
void hr_join(const relation_t * const r_rel, const relation_t * const s_rel, relation_t * const r_tmp, join_arg_t * const join_arg) {
	join_arg -> matched_cnt = 0;
	join_arg -> checksum = 0;
...
void hro_join(const relation_t * const r_rel, const relation_t * const s_rel, relation_t * const r_tmp, join_arg_t * const join_arg) {
	join_arg -> matched_cnt = 0;
	join_arg -> checksum = 0;
...
void *prj_thread_buildpart(void *param) {
	phj_arg_t *phj_arg = (phj_arg_t *) param;
	...
	parallel_radix_partition_optimized(&part, swwcb, FANOUT_PASS2 * PADDING_UNIT_NUM, 0);
#else /* USE_SWWCB_OPTIMIZED_PART */
	parallel_radix_partition(&part, FANOUT_PASS2 * PADDING_UNIT_NUM);
#endif /* USE_SWWCB_OPTIMIZED_PART */
	...
	while ( ( task = task_queue_get_atomic(part_queue) ) ) {
		serial_radix_partition(task, phj_arg->join_queue, 
	...
	while(1) {
		...
		parallel_radix_partition_optimized(&part, skew_hand_swwcb, PADDING_UNIT_NUM, (*skewtask)->r_global_offset);

#else /* USE_SWWCB_OPTIMIZED_PART */
		parallel_radix_partition(&part, PADDING_UNIT_NUM);
#endif /* USE_SWWCB_OPTIMIZED_PART */
		...
		parallel_radix_partition_optimized(&part, skew_hand_swwcb, PADDING_UNIT_NUM, (*skewtask)->s_global_offset);
...
void *prj_thread_probejoin(void *param) {
	phj_arg_t *phj_arg = (phj_arg_t *) param;
	join_arg_t join_arg;
...
void join_init_run(const datameta_t datameta, timekeeper_t * const timekeeper, 
	...
	for (int idx = 0; idx < BUILDPART_THREAD_NUM; idx ++) {
		...
		rv = pthread_create(&threadpool[idx], &attr, prj_thread_buildpart, args+idx);
		if (rv){
	...
	for (int idx = 0; idx < PROBEJOIN_THREAD_NUM; idx ++) {
		...
		rv = pthread_create(&threadpool[idx], &attr, prj_thread_probejoin, args+idx);
		if (rv){
			printf("ERROR; return code from pthread_create() is %d\n", rv);
			exit(EXIT_FAILURE);
		}
...
void phj_radix_bc(const datameta_t datameta, timekeeper_t * const timekeeper) {
	join_init_run(datameta, timekeeper, bc_join);
}

void phj_radix_lp(const datameta_t datameta, timekeeper_t * const timekeeper) {
	join_init_run(datameta, timekeeper, lp_join);
}

void phj_radix_sc(const datameta_t datameta, timekeeper_t * const timekeeper) {
	join_init_run(datameta, timekeeper, sc_join);
}

void phj_radix_hr(const datameta_t datameta, timekeeper_t * const timekeeper) {
	join_init_run(datameta, timekeeper, hr_join);
}

#if TUPLE_T_SIZE <= AVX512_SIZE
void phj_radix_hro(const datameta_t datameta, timekeeper_t * const timekeeper) {
	join_init_run(datameta, timekeeper, hro_join);
}
#endif /* TUPLE_T_SIZE <= AVX512_SIZE */