...
extern int cur_t;
typedef void (*ptr_joinfunction_t)(const ptr_relation_t * const, const ptr_relation_t * const, ptr_relation_t * const, ptr_join_arg_t * const);


/** holds the arguments passed to each thread */
typedef struct ptr_phj_arg_t {
	int _tid;
	...
	ptr_joinfunction_t joinfunction;
...
#ifdef SYNCSTATS
	ptr_synctimer_phj_t *localtimer;
	ptr_synctimer_phj_t *globaltimer;
#ifdef SKEW_HANDLING
...
void ptr_parallel_radix_partition_first(ptr_part_t * const part, const size_t padding_num) {
	int rv;
	...
	for (size_t idx = 0; idx < rel.tuple_num; idx ++) {	
		tmp_hashkey = IDHASH(rel.tuples[idx].key, hashmask, bitskip);
		store_a_keyrid(& tmp.keyrids[ output_offset[tmp_hashkey] + tmp_hashkey * padding_num] , & rel.tuples[idx]);
...
#ifdef SKEW_HANDLING
void ptr_parallel_radix_partition_following(ptr_part_t * const part, const size_t padding_num) {
	int rv;
	...
	/* copy keyrids to their corresponding clusters */
	for (size_t idx = 0; idx < rel.keyrid_num; idx ++) {	
		tmp_hashkey = IDHASH(rel.keyrids[idx].key, hashmask, bitskip);
		store_a_keyrid(& tmp.keyrids[ output_offset[tmp_hashkey] + tmp_hashkey * padding_num] , & rel.keyrids[idx]);
		output_offset[tmp_hashkey] ++;
	}
...
void ptr_parallel_radix_partition_optimized_first(ptr_part_t * const part, ptr_swwcb_t * const swwcb, 
	...
	for (size_t idx = 0; idx < rel.tuple_num; idx ++) {
		...
		memcpy(&tmp_swwcb[slot_mod], &rel.tuples[idx], KEYRID_T_SIZE);

		if (slot_mod == KEYRID_PER_SWWCB - 1) {
			/* non-temporal store a SWWCB */
			nontemp_store_swwcb( & tmp.keyrids[slot-(KEYRID_PER_SWWCB-1)-global_offset] , tmp_swwcb);
		}
	...
	for (size_t idx = 0; idx < fanout; idx ++) {
		...
		for (size_t jdx = remainder_start_pos; jdx < slot_mod; jdx ++) {
			store_a_keyrid(& tmp.keyrids[slot+jdx-global_offset] , & swwcb[idx].data.keyrids[jdx]);
		}
	}
}
...
#ifdef SKEW_HANDLING
void ptr_parallel_radix_partition_optimized_following(ptr_part_t * const part, ptr_swwcb_t * const swwcb, 
	const size_t padding_num, const size_t global_offset) {
	int rv;
...
void ptr_radix_cluster(ptr_relation_t out_rel, ptr_relation_t in_rel, size_t *hist, 
...
void ptr_radix_cluster_optimized(ptr_relation_t out_rel, ptr_relation_t in_rel, size_t *hist, const size_t fanout, 
	const key_t_ bitskip, ptr_swwcb_t * const swwcb, const size_t padding_num, const size_t global_offset) {
	key_t_ hashmask = (fanout - 1) << bitskip;
...
void ptr_serial_radix_partition(ptr_task_t * const task, ptr_task_queue_t * join_queue, const size_t fanout, const key_t_ bitskip, 
...
#ifdef PTR_USE_SWWCB_OPTIMIZED_RADIX_CLUSTER
	ptr_swwcb_t *swwcb = (ptr_swwcb_t *) swwcb_mem;
	ptr_radix_cluster_optimized(task->r_tmp, task->r_rel, r_hist, fanout, bitskip, swwcb, padding_num, task->r_global_offset);
	ptr_radix_cluster_optimized(task->s_tmp, task->s_rel, s_hist, fanout, bitskip, swwcb, padding_num, task->s_global_offset);
#else /* PTR_USE_SWWCB_OPTIMIZED_RADIX_CLUSTER_ON_NVM */
	ptr_radix_cluster(task->r_tmp, task->r_rel, r_hist, fanout, bitskip, padding_num);
	ptr_radix_cluster(task->s_tmp, task->s_rel, s_hist, fanout, bitskip, padding_num);
	...
	for (size_t idx = 0; idx < fanout; idx ++) {
		if (r_hist[idx*2] > 0 && s_hist[idx*2] > 0) {
			...
			tmp_task->r_rel.keyrid_num = r_hist[idx*2];
			...
			tmp_task->s_rel.keyrid_num = s_hist[idx*2];
...
void ptr_bc_join(const ptr_relation_t * const r_rel, const ptr_relation_t * const s_rel, ptr_relation_t * const r_tmp, ptr_join_arg_t * const join_arg) {
...
void ptr_lp_join(const ptr_relation_t * const r_rel, const ptr_relation_t * const s_rel, ptr_relation_t * const r_tmp, ptr_join_arg_t * const join_arg) {
	join_arg -> matched_cnt = 0;
#ifdef NONTEMP_STORE_MATER
	size_t slot;
	size_t slot_mod;
...
void ptr_sc_join(const ptr_relation_t * const r_rel, const ptr_relation_t * const s_rel, ptr_relation_t * const r_tmp, ptr_join_arg_t * const join_arg) {
	join_arg -> matched_cnt = 0;
#ifdef NONTEMP_STORE_MATER
	size_t slot;
	size_t slot_mod;
...
void ptr_hr_join(const ptr_relation_t * const r_rel, const ptr_relation_t * const s_rel, ptr_relation_t * const r_tmp, ptr_join_arg_t * const join_arg) {
	join_arg -> matched_cnt = 0;
#ifdef NONTEMP_STORE_MATER
	size_t slot;
	size_t slot_mod;
...
// void ptr_hro_join(const ptr_relation_t * const r_rel, const ptr_relation_t * const s_rel, ptr_relation_t * const r_tmp, ptr_join_arg_t * const join_arg) {
// 	join_arg -> matched_cnt = 0;
// 	join_arg -> checksum = 0;
// 	// row_id_pair_t tmp_row_id_pair;
...
void *ptr_phj_thread_buildpart(void * param) {
	ptr_phj_arg_t *phj_arg = (ptr_phj_arg_t *) param;
	...
	swwcb = (ptr_swwcb_t *) new_alloc_nvm(sizeof(ptr_swwcb_t) * FANOUT_PASS1);
#else /* PTR_USE_SWWCB_OPTIMIZED_PART_ON_NVM */
	swwcb = (ptr_swwcb_t *) alloc_aligned_dram(sizeof(ptr_swwcb_t) * FANOUT_PASS1, CACHELINE_SIZE);
	...
	ptr_parallel_radix_partition_optimized_first(&part, swwcb, FANOUT_PASS2 * PTR_PADDING_UNIT_NUM, 0);
#else /* PTR_USE_SWWCB_OPTIMIZED_PART */
	ptr_parallel_radix_partition_first(&part, FANOUT_PASS2 * PTR_PADDING_UNIT_NUM);
#endif /* PTR_USE_SWWCB_OPTIMIZED_PART */
	...
	ser_swwcb = new_alloc_nvm(sizeof(ptr_swwcb_t) * FANOUT_PASS2);
	pmem_memset_persist(ser_swwcb, 0, sizeof(ptr_swwcb_t) * FANOUT_PASS2);
#else /* PTR_USE_SWWCB_OPTIMIZED_RADIX_CLUSTER_ON_NVM */
	ser_swwcb = alloc_aligned_dram(sizeof(ptr_swwcb_t) * FANOUT_PASS2, CACHELINE_SIZE);
	memset(ser_swwcb, 0, sizeof(ptr_swwcb_t) * FANOUT_PASS2);
#endif /* PTR_USE_SWWCB_OPTIMIZED_RADIX_CLUSTER_ON_NVM */
#endif /* PTR_USE_SWWCB_OPTIMIZED_RADIX_CLUSTER */
	...
	while ( ( task = task_queue_get_atomic(part_queue) ) ) {
		ptr_serial_radix_partition(task, phj_arg->join_queue, 
			FANOUT_PASS2, NUM_RADIX_BITS / NUM_PASSES, 
			ser_r_hist, ser_s_hist, ser_swwcb, &max_build_side_tup_num, PTR_PADDING_UNIT_NUM
		);
	}
...
#ifdef PTR_USE_SWWCB_OPTIMIZED_RADIX_CLUSTER
#ifdef PTR_USE_SWWCB_OPTIMIZED_RADIX_CLUSTER_ON_NVM
	pmem_unmap(ser_swwcb, sizeof(ptr_swwcb_t) * FANOUT_PASS2);
#else /* PTR_USE_SWWCB_OPTIMIZED_RADIX_CLUSTER_ON_NVM */
	free(ser_swwcb);
#endif /* PTR_USE_SWWCB_OPTIMIZED_RADIX_CLUSTER_ON_NVM */
#endif /* PTR_USE_SWWCB_OPTIMIZED_RADIX_CLUSTER */
...
#ifdef PTR_USE_SWWCB_OPTIMIZED_PART
	ptr_swwcb_t *skew_hand_swwcb;
#ifdef PTR_USE_SWWCB_OPTIMIZED_PART_ON_NVM
	skew_hand_swwcb = (ptr_swwcb_t *) new_alloc_nvm(sizeof(ptr_swwcb_t) * FANOUT_PASS2);
	pmem_memset_persist(skew_hand_swwcb, 0, sizeof(ptr_swwcb_t) * FANOUT_PASS2);
#else /* PTR_USE_SWWCB_OPTIMIZED_PART_ON_NVM */
	skew_hand_swwcb = (ptr_swwcb_t *) alloc_aligned_dram(sizeof(ptr_swwcb_t) * FANOUT_PASS2, CACHELINE_SIZE);
	memset(skew_hand_swwcb, 0, sizeof(ptr_swwcb_t) * FANOUT_PASS2);
#endif /* PTR_USE_SWWCB_OPTIMIZED_PART_ON_NVM */
#endif /* PTR_USE_SWWCB_OPTIMIZED_PART */
	...
	size_t skew_hand_large_task_num_keyrid;
	...
	while(1) {
		...
		part.ext.keyrid_num = (phj_arg->_tid == BUILDPART_THREAD_NUM -1) ? 
			((*skewtask)->r_rel.keyrid_num - phj_arg->_tid * skew_hand_num_per_thr) : skew_hand_num_per_thr;
		...
		ptr_parallel_radix_partition_optimized_following(&part, skew_hand_swwcb, PTR_PADDING_UNIT_NUM, (*skewtask)->r_global_offset);

#else /* PTR_USE_SWWCB_OPTIMIZED_PART */
		ptr_parallel_radix_partition_following(&part, PTR_PADDING_UNIT_NUM);
#endif /* PTR_USE_SWWCB_OPTIMIZED_PART */
		...
		part.ext.keyrid_num = (phj_arg->_tid == BUILDPART_THREAD_NUM -1) ? 
			((*skewtask)->s_rel.keyrid_num - phj_arg->_tid * skew_hand_num_per_thr) : skew_hand_num_per_thr;
		...
		ptr_parallel_radix_partition_optimized_following(&part, skew_hand_swwcb, PTR_PADDING_UNIT_NUM, (*skewtask)->s_global_offset);
		...
		if (phj_arg -> _tid == 0) {
			...
			for (size_t idx = 0; idx < FANOUT_PASS2; idx ++) {
				...
				skew_hand_r_num_keyrid = (idx == FANOUT_PASS2 - 1) ?
					(*skewtask)->r_rel.keyrid_num - skew_hand_r_output_offset[idx] + idx * PTR_PADDING_UNIT_NUM :
					skew_hand_r_output_offset[idx+1] - skew_hand_r_output_offset[idx] - PTR_PADDING_UNIT_NUM;
				skew_hand_s_num_keyrid = (idx == FANOUT_PASS2 - 1) ?
					(*skewtask)->s_rel.keyrid_num - skew_hand_s_output_offset[idx] + idx * PTR_PADDING_UNIT_NUM :
					skew_hand_s_output_offset[idx+1] - skew_hand_s_output_offset[idx] - PTR_PADDING_UNIT_NUM;
				...
				if (skew_hand_r_num_keyrid > THRESHOLD_1 || skew_hand_s_num_keyrid > THRESHOLD_1) {
					for (size_t jdx = 0; jdx < PROBEJOIN_THREAD_NUM; jdx ++) {
						...
						skew_hand_task->r_rel.keyrids = (*skewtask)->r_tmp.keyrids + skew_hand_r_output_offset[idx];
						skew_hand_task->r_tmp.keyrids = (*skewtask)->r_rel.keyrids + skew_hand_r_output_offset[idx];

						skew_hand_large_task_num_keyrid = (jdx == PROBEJOIN_THREAD_NUM - 1) ?
						...
						skew_hand_task->s_rel.keyrids = (*skewtask)->s_tmp.keyrids + skew_hand_s_output_offset[idx] + jdx * ( skew_hand_s_num_keyrid / PROBEJOIN_THREAD_NUM );
						skew_hand_task->s_tmp.keyrids = (*skewtask)->s_rel.keyrids + skew_hand_s_output_offset[idx] + jdx * ( skew_hand_s_num_keyrid / PROBEJOIN_THREAD_NUM );
					...
					if (skew_hand_r_num_keyrid > 0 && skew_hand_s_num_keyrid > 0) {
						...
						skew_hand_task->s_rel.keyrids = (*skewtask)->s_tmp.keyrids + skew_hand_s_output_offset[idx];
						skew_hand_task->s_tmp.keyrids = (*skewtask)->s_rel.keyrids + skew_hand_s_output_offset[idx];
...
#ifdef PTR_USE_SWWCB_OPTIMIZED_PART
#ifdef PTR_USE_SWWCB_OPTIMIZED_PART_ON_NVM
	pmem_unmap(skew_hand_swwcb, sizeof(ptr_swwcb_t) * FANOUT_PASS2);
#else /* PTR_USE_SWWCB_OPTIMIZED_PART_ON_NVM */
...
void * ptr_phj_thread_probejoin(void * param) {
	ptr_phj_arg_t *phj_arg = (ptr_phj_arg_t *) param;
...
void * ptr_phj_thread_retrieve(void * param) {
	ptr_phj_arg_t *phj_arg = (ptr_phj_arg_t *) param;
...
void ptr_join_init_run(const datameta_t datameta, timekeeper_t * const timekeeper, 
	...
	ptr_phj_arg_t args[max_thread_num];
...
#ifdef SYNCSTATS
	ptr_synctimer_phj_t *localtimer = (ptr_synctimer_phj_t *) malloc( max_thread_num * sizeof(ptr_synctimer_phj_t) );
	ptr_synctimer_phj_t *globaltimer = (ptr_synctimer_phj_t *) malloc( sizeof(ptr_synctimer_phj_t) );
	memset(localtimer, 0, max_thread_num * sizeof(ptr_synctimer_phj_t));
	memset(globaltimer, 0, sizeof(ptr_synctimer_phj_t));
#endif /* SYNCSTATS */
	...
	for (int idx = 0; idx < BUILDPART_THREAD_NUM; idx ++) {
		...
		rv = pthread_create(&threadpool[idx], &attr, ptr_phj_thread_buildpart, args+idx);
		if (rv){
	...
	for (int idx = 0; idx < PROBEJOIN_THREAD_NUM; idx ++) {
		...
		rv = pthread_create(&threadpool[idx], &attr, ptr_phj_thread_probejoin, args+idx);
		if (rv){
			printf("ERROR; return code from pthread_create() is %d\n", rv);
			exit(EXIT_FAILURE);
		}
	...
	for (int idx = 0; idx < RETRIEVE_THREAD_NUM; idx ++) {
		...
		rv = pthread_create(&threadpool[idx], &attr, ptr_phj_thread_retrieve, args+idx);
		if (rv){
			printf("ERROR; return code from pthread_create() is %d\n", rv);
			exit(EXIT_FAILURE);
		}
...
void ptr_phj_radix_bc(const datameta_t datameta, timekeeper_t * const timekeeper) {
	ptr_join_init_run(datameta, timekeeper, ptr_bc_join);
}

void ptr_phj_radix_lp(const datameta_t datameta, timekeeper_t * const timekeeper) {
	ptr_join_init_run(datameta, timekeeper, ptr_lp_join);
}

void ptr_phj_radix_sc(const datameta_t datameta, timekeeper_t * const timekeeper) {
	ptr_join_init_run(datameta, timekeeper, ptr_sc_join);
}

void ptr_phj_radix_hr(const datameta_t datameta, timekeeper_t * const timekeeper) {
	ptr_join_init_run(datameta, timekeeper, ptr_hr_join);
}

// void ptr_phj_radix_hro(const datameta_t datameta, timekeeper_t * const timekeeper) {
// 	ptr_join_init_run(datameta, timekeeper, ptr_hro_join);
// }
