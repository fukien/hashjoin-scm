...
#include "phj_asymm_radix_bw_reg.h"
...
extern int cur_t;
typedef void (*asymm_radix_joinfunction_bw_reg_t)(const relation_t * const, const relation_t * const, relation_t * const, join_arg_t * const);


/** holds the arguments passed to each thread */
typedef struct phj_asymm_radix_arg_bw_reg_t {
	int _tid;
	...
	asymm_radix_joinfunction_bw_reg_t joinfunction;
	...
	swwcb_t **swwcb_s;
#endif /* USE_SWWCB_OPTIMIZED_PART */	
} phj_asymm_radix_arg_bw_reg_t;


typedef struct asymm_radix_part_bw_reg_t {
	int _tid;
	pthread_barrier_t *barrier;
...
} asymm_radix_part_bw_reg_t;



void asymm_radix_parallel_radix_partition_bw_reg(asymm_radix_part_bw_reg_t * const part, const size_t padding_num) {
	int rv;
...
#ifdef USE_SWWCB_OPTIMIZED_PART
void asymm_radix_parallel_radix_partition_optimized_bw_reg(asymm_radix_part_bw_reg_t * const part,
...
void asymm_radix_radix_cluster_bw_reg(relation_t out_rel, relation_t in_rel, const size_t fanout, 
...
#ifdef USE_SWWCB_OPTIMIZED_RADIX_CLUSTER
void asymm_radix_radix_cluster_optimized_bw_reg(relation_t out_rel, relation_t in_rel, 
...
void asymm_radix_serial_radix_partition_bw_reg(task_t * const task, task_queue_t * join_queue, task_queue_t * part_queue_2nd_pass, 
	...
	if (before_mid_hist_flag) {
		...
		tmp_r_hist_2nd_pass = r_hist_2nd_pass + tmp_idx_2nd_pass * 2 * FANOUT_PASS2_ASYMM;
#ifdef USE_SWWCB_OPTIMIZED_RADIX_CLUSTER
		tmp_r_swwcb_2nd_pass_mem = r_swwcb_2nd_pass_mem + tmp_idx_2nd_pass * FANOUT_PASS2_ASYMM;
#endif /* USE_SWWCB_OPTIMIZED_RADIX_CLUSTER */
	} else {
...
#ifdef USE_SWWCB_OPTIMIZED_RADIX_CLUSTER
	asymm_radix_radix_cluster_optimized_bw_reg(task->r_tmp, task->r_rel, fanout, bitskip, padding_num, 
	...
	asymm_radix_radix_cluster_bw_reg(task->r_tmp, task->r_rel, fanout, bitskip, padding_num, 
	...
	if (_tid < BUILDPART_WRITE_THREAD_NUM) {
		...
		for (size_t idx = 0; idx < fanout; idx ++) {
			if (tmp_r_hist_2nd_pass[idx*2] > 0) {
...
void asymm_radix_bc_bw_reg_join(const relation_t * const r_rel, const relation_t * const s_rel, relation_t * const r_tmp, join_arg_t * const join_arg) {
...
void asymm_radix_sc_bw_reg_join(const relation_t * const r_rel, const relation_t * const s_rel, relation_t * const r_tmp, join_arg_t * const join_arg) {
	join_arg -> matched_cnt = 0;
	join_arg -> checksum = 0;
...
void asymm_radix_hr_bw_reg_join(const relation_t * const r_rel, const relation_t * const s_rel, relation_t * const r_tmp, join_arg_t * const join_arg) {
	join_arg -> matched_cnt = 0;
	join_arg -> checksum = 0;
	...
	for (size_t idx = 0; idx < s_rel->tuple_num; idx ++) {
		tmp_hashkey = IDHASH(s_rel->tuples[idx].key, hashmask, NUM_RADIX_BITS);
		start_idx = (tmp_hashkey != 0) * hist[ tmp_hashkey - (tmp_hashkey != 0) ];
...
void asymm_radix_hro_bw_reg_join(const relation_t * const r_rel, const relation_t * const s_rel, relation_t * const r_tmp, join_arg_t * const join_arg) {
	join_arg -> matched_cnt = 0;
	join_arg -> checksum = 0;
...
void *prj_asymm_radix_thread_buildpart_bw_reg(void *param) {
	phj_asymm_radix_arg_bw_reg_t *phj_arg = (phj_asymm_radix_arg_bw_reg_t *) param;
...
#endif /* SKEW_HANDLING */
	asymm_radix_part_bw_reg_t part;

	part._tid = phj_arg->_tid;
	part.barrier = phj_arg->barrier;

	phj_arg->r_hist[phj_arg->_tid] = (size_t *) alloc_memory( sizeof(size_t) * 2 * FANOUT_PASS1_ASYMM, CACHELINE_SIZE);
	phj_arg->s_hist[phj_arg->_tid] = (size_t *) alloc_memory( sizeof(size_t) * 2 * FANOUT_PASS1_ASYMM, CACHELINE_SIZE);
	
#ifdef OUTPUT_OFFSET_NVM
	phj_arg->r_output_offset[phj_arg->_tid] = (size_t *) new_alloc_nvm( sizeof(size_t) * FANOUT_PASS1_ASYMM);
	phj_arg->s_output_offset[phj_arg->_tid] = (size_t *) new_alloc_nvm( sizeof(size_t) * FANOUT_PASS1_ASYMM);
#else /* OUTPUT_OFFSET_NVM */
	phj_arg->r_output_offset[phj_arg->_tid] = (size_t *) alloc_dram( sizeof(size_t) * FANOUT_PASS1_ASYMM, CACHELINE_SIZE);
	phj_arg->s_output_offset[phj_arg->_tid] = (size_t *) alloc_dram( sizeof(size_t) * FANOUT_PASS1_ASYMM, CACHELINE_SIZE);
	...
	phj_arg->swwcb_s[phj_arg->_tid] = (swwcb_t *) new_alloc_nvm(sizeof(swwcb_t) * FANOUT_PASS1_ASYMM);
#else /* USE_SWWCB_OPTIMIZED_PART_ON_NVM */
	phj_arg->swwcb_s[phj_arg->_tid] = (swwcb_t *) alloc_aligned_dram(sizeof(swwcb_t) * FANOUT_PASS1_ASYMM, CACHELINE_SIZE);
	...
	memset_localize(phj_arg->r_hist[phj_arg->_tid] , sizeof(size_t) * 2 * FANOUT_PASS1_ASYMM);
	memset_localize(phj_arg->s_hist[phj_arg->_tid] , sizeof(size_t) * 2 * FANOUT_PASS1_ASYMM);

#ifdef OUTPUT_OFFSET_NVM
	pmem_memset_persist(r_output_offset, 0, sizeof(size_t) * FANOUT_PASS1_ASYMM);
	pmem_memset_persist(s_output_offset, 0, sizeof(size_t) * FANOUT_PASS1_ASYMM);
#else /* OUTPUT_OFFSET_NVM */
	memset(r_output_offset, 0, sizeof(size_t) * FANOUT_PASS1_ASYMM);
	memset(s_output_offset, 0, sizeof(size_t) * FANOUT_PASS1_ASYMM);
#endif /* OUTPUT_OFFSET_NVM */

#ifdef USE_SWWCB_OPTIMIZED_PART
#ifdef USE_SWWCB_OPTIMIZED_PART_ON_NVM
	pmem_memset_persist(swwcb, 0, sizeof(swwcb_t) * FANOUT_PASS1_ASYMM);
#else /* USE_SWWCB_OPTIMIZED_PART_ON_NVM */
	memset(swwcb, 0, sizeof(swwcb_t) * FANOUT_PASS1_ASYMM);
#endif /* USE_SWWCB_OPTIMIZED_PART_ON_NVM */
#endif /* USE_SWWCB_OPTIMIZED_PART */
	...
	part.bitskip = 0;
	part.hashmask = FANOUT_PASS1_ASYMM-1;
#ifdef USE_SWWCB_OPTIMIZED_PART
	...
	asymm_radix_parallel_radix_partition_optimized_bw_reg(&part, FANOUT_PASS2_ASYMM * PADDING_UNIT_NUM, 0);
#else /* USE_SWWCB_OPTIMIZED_PART */
	asymm_radix_parallel_radix_partition_bw_reg(&part, FANOUT_PASS2_ASYMM * PADDING_UNIT_NUM);
#endif /* USE_SWWCB_OPTIMIZED_PART */
	...
	size_t s_num_tup;
	if (phj_arg->_tid == 0) {
		for (size_t idx = 0; idx < FANOUT_PASS1_ASYMM; idx ++) {

			/* within the first thread, thus no memory leak */
			r_num_tup = (idx == (FANOUT_PASS1_ASYMM-1)) ?
				phj_arg->datameta->r_tuple_num - r_output_offset[idx] + idx * FANOUT_PASS2_ASYMM * PADDING_UNIT_NUM :
				r_output_offset[idx+1] - r_output_offset[idx] - FANOUT_PASS2_ASYMM * PADDING_UNIT_NUM;

			s_num_tup = (idx == (FANOUT_PASS1_ASYMM-1)) ?
				phj_arg->datameta->s_tuple_num - s_output_offset[idx] + idx * FANOUT_PASS2_ASYMM * PADDING_UNIT_NUM :
				s_output_offset[idx+1] - s_output_offset[idx] - FANOUT_PASS2_ASYMM * PADDING_UNIT_NUM;
	...
	while ( ( task = task_queue_get_atomic(part_queue) ) ) {
		asymm_radix_serial_radix_partition_bw_reg(task, phj_arg->join_queue, 
			phj_arg->part_queue_2nd_pass, FANOUT_PASS2_ASYMM, 
			NUM_RADIX_BITS_PASS1, &max_build_side_tup_num, 
	...
	if (phj_arg->_tid < BUILDPART_WRITE_THREAD_NUM) {
		while ( ( task = task_queue_get_atomic(phj_arg->part_queue_2nd_pass) ) ) {
			asymm_radix_serial_radix_partition_bw_reg(task, phj_arg->join_queue, 
				NULL, FANOUT_PASS2_ASYMM, NUM_RADIX_BITS_PASS1, 
	...
	part.bitskip = NUM_RADIX_BITS_PASS1;
	part.hashmask = FANOUT_PASS2_ASYMM-1;

	dealloc_memory(phj_arg->r_output_offset[phj_arg->_tid], sizeof(size_t) * FANOUT_PASS1_ASYMM);
#ifdef OUTPUT_OFFSET_NVM
	phj_arg->r_output_offset[phj_arg->_tid] = (size_t *) new_alloc_nvm( sizeof(size_t) * FANOUT_PASS2_ASYMM );
#else /* OUTPUT_OFFSET_NVM */
	phj_arg->r_output_offset[phj_arg->_tid] = (size_t *) alloc_dram( sizeof(size_t) * FANOUT_PASS2_ASYMM, CACHELINE_SIZE);
#endif /* OUTPUT_OFFSET_NVM */
	r_output_offset = phj_arg->r_output_offset[phj_arg->_tid];

#ifdef USE_SWWCB_OPTIMIZED_PART
	dealloc_memory( phj_arg->swwcb_s[phj_arg->_tid], sizeof(swwcb_t) * FANOUT_PASS1_ASYMM);
#ifdef USE_SWWCB_OPTIMIZED_PART_ON_NVM
	phj_arg->swwcb_s[phj_arg->_tid] = (swwcb_t *) new_alloc_nvm(sizeof(swwcb_t) * FANOUT_PASS2_ASYMM);
	pmem_memset_persist(phj_arg->swwcb_s[phj_arg->_tid], 0, sizeof(swwcb_t) * FANOUT_PASS2_ASYMM);
#else /* USE_SWWCB_OPTIMIZED_PART_ON_NVM */
	phj_arg->swwcb_s[phj_arg->_tid] = (swwcb_t *) alloc_aligned_dram(sizeof(swwcb_t) * FANOUT_PASS2_ASYMM, CACHELINE_SIZE);
	memset(phj_arg->swwcb_s[phj_arg->_tid], 0, sizeof(swwcb_t) * FANOUT_PASS2_ASYMM);
#endif /* USE_SWWCB_OPTIMIZED_PART_ON_NVM */
	swwcb = phj_arg->swwcb_s[phj_arg->_tid];
#endif /* USE_SWWCB_OPTIMIZED_PART */

	dealloc_memory(phj_arg->r_hist[phj_arg->_tid], sizeof(size_t) * 2 * FANOUT_PASS1_ASYMM);

	phj_arg->r_hist[phj_arg->_tid] = (size_t *) alloc_memory( sizeof(size_t) * 2 * FANOUT_PASS2_ASYMM, CACHELINE_SIZE);

	size_t skew_hand_num_per_thr;
	size_t skew_hand_r_num_tup;
	...
	while(1) {
		...
		phj_arg->sync_idle_skew_prfx_sum_acc[0] += diff_sec(phj_arg->localtimer->sync_skew_hist[0],
			phj_arg->globaltimer->sync_skew_hist[0]);
		phj_arg->sync_idle_skew_part_acc += diff_sec(phj_arg->localtimer->sync_skew_prfx_sum[0],
		...
		memset_localize(phj_arg->r_hist[phj_arg->_tid], sizeof(size_t) * 2 * FANOUT_PASS2_ASYMM);

		/* explicitly zero the skew_hand offset array */
#ifdef OUTPUT_OFFSET_NVM
		pmem_memset_persist(phj_arg->r_output_offset[phj_arg->_tid], 0, sizeof(size_t) * FANOUT_PASS2_ASYMM );
#else /* OUTPUT_OFFSET_NVM */
		memset(phj_arg->r_output_offset[phj_arg->_tid], 0, sizeof(size_t) * FANOUT_PASS2_ASYMM );
#endif /* OUTPUT_OFFSET_NVM */
		...
		asymm_radix_parallel_radix_partition_optimized_bw_reg(&part, PADDING_UNIT_NUM, (*skewtask)->r_global_offset);

#else /* USE_SWWCB_OPTIMIZED_PART */
		asymm_radix_parallel_radix_partition_bw_reg(&part, PADDING_UNIT_NUM);
#endif /* USE_SWWCB_OPTIMIZED_PART */
		...
		if (phj_arg -> _tid == 0) {
			...
			for (size_t idx = 0; idx < FANOUT_PASS2_ASYMM; idx ++) {

				/* within the first thread, thus no memory leak */
				skew_hand_r_num_tup = (idx == FANOUT_PASS2_ASYMM - 1) ?
				...
				if (skew_hand_r_num_tup > THRESHOLD_1) {
					for (size_t jdx = 0; jdx < PROBEJOIN_THREAD_NUM; jdx ++) {
						...
						size_t skew_hand_large_task_num_tup = (jdx == PROBEJOIN_THREAD_NUM - 1) ?
							(*skewtask)->s_rel.tuple_num - jdx * ( (*skewtask)->s_rel.tuple_num / PROBEJOIN_THREAD_NUM) :
							(*skewtask)->s_rel.tuple_num / PROBEJOIN_THREAD_NUM;

						skew_hand_task->s_rel.tuple_num = skew_hand_task->s_tmp.tuple_num = skew_hand_large_task_num_tup;

						skew_hand_task->s_rel.tuples = (*skewtask)->s_rel.tuples + jdx * ( (*skewtask)->s_rel.tuple_num / PROBEJOIN_THREAD_NUM );
						skew_hand_task->s_tmp.tuples = (*skewtask)->s_tmp.tuples + jdx * ( (*skewtask)->s_rel.tuple_num / PROBEJOIN_THREAD_NUM );

						task_queue_add(part_queue, skew_hand_task);
					}

				} else {
					if (skew_hand_r_num_tup > 0) {
						...
						skew_hand_task->s_rel.tuple_num = skew_hand_task->s_tmp.tuple_num = (*skewtask)->s_rel.tuple_num;
						skew_hand_task->s_rel.tuples = (*skewtask)->s_rel.tuples;
						skew_hand_task->s_tmp.tuples = (*skewtask)->s_tmp.tuples;
...
#ifdef SKEW_HANDLING
	dealloc_memory(phj_arg->r_hist[phj_arg->_tid], sizeof(size_t) * 2 * FANOUT_PASS2_ASYMM);
	dealloc_memory(phj_arg->s_hist[phj_arg->_tid], sizeof(size_t) * 2 * FANOUT_PASS2_ASYMM);
#ifdef USE_SWWCB_OPTIMIZED_PART
#ifdef USE_SWWCB_OPTIMIZED_PART_ON_NVM
	pmem_unmap(swwcb, sizeof(swwcb_t) * FANOUT_PASS2_ASYMM);
#else /* USE_SWWCB_OPTIMIZED_PART_ON_NVM */
	free(swwcb);
#endif /* USE_SWWCB_OPTIMIZED_PART_ON_NVM */
#endif /* USE_SWWCB_OPTIMIZED_PART */
#ifdef OUTPUT_OFFSET_NVM
	pmem_unmap(r_output_offset, sizeof(size_t) * FANOUT_PASS2_ASYMM);
	pmem_unmap(s_output_offset, sizeof(size_t) * FANOUT_PASS2_ASYMM);
#else /* OUTPUT_OFFSET_NVM */
	dealloc_dram(r_output_offset, sizeof(size_t) * FANOUT_PASS2_ASYMM);
	dealloc_dram(s_output_offset, sizeof(size_t) * FANOUT_PASS2_ASYMM);
#endif /* OUTPUT_OFFSET_NVM */
#else /* SKEW_HANDLING */
	dealloc_memory(phj_arg->r_hist[phj_arg->_tid], sizeof(size_t) * 2 * FANOUT_PASS1_ASYMM);
	dealloc_memory(phj_arg->s_hist[phj_arg->_tid], sizeof(size_t) * 2 * FANOUT_PASS1_ASYMM);
#ifdef USE_SWWCB_OPTIMIZED_PART
#ifdef USE_SWWCB_OPTIMIZED_PART_ON_NVM
	pmem_unmap(swwcb, sizeof(swwcb_t) * FANOUT_PASS1_ASYMM);
#else /* USE_SWWCB_OPTIMIZED_PART_ON_NVM */
	free(swwcb);
#endif /* USE_SWWCB_OPTIMIZED_PART_ON_NVM */
#endif /* USE_SWWCB_OPTIMIZED_PART */
#ifdef OUTPUT_OFFSET_NVM
	pmem_unmap(r_output_offset, sizeof(size_t) * FANOUT_PASS1_ASYMM);
	pmem_unmap(s_output_offset, sizeof(size_t) * FANOUT_PASS1_ASYMM);
#else /* OUTPUT_OFFSET_NVM */
	dealloc_dram(r_output_offset, sizeof(size_t) * FANOUT_PASS1_ASYMM);
	dealloc_dram(s_output_offset, sizeof(size_t) * FANOUT_PASS1_ASYMM);
#endif /* OUTPUT_OFFSET_NVM */
#endif /* SKEW_HANDLING */
...
void *prj_asymm_radix_thread_probejoin_bw_reg(void *param) {
	phj_asymm_radix_arg_bw_reg_t *phj_arg = (phj_asymm_radix_arg_bw_reg_t *) param;
	join_arg_t join_arg;
...
void asymm_radix_join_init_run_bw_reg(const datameta_t datameta, timekeeper_t * const timekeeper, 
	...
	task_queue_t *part_queue = task_queue_init(FANOUT_PASS1_ASYMM);
	task_queue_t *join_queue = task_queue_init(1 << NUM_RADIX_BITS);

#if NUM_PASSES != 1
	idx_2nd_pass = 0;

	task_queue_t *part_queue_2nd_pass = task_queue_init(FANOUT_PASS1_ASYMM);
	r_hist_2nd_pass = (size_t *) alloc_memory( sizeof(size_t) * 2 * FANOUT_PASS2_ASYMM * FANOUT_PASS1_ASYMM, CACHELINE_SIZE);
	// s_hist_2nd_pass = (size_t *) alloc_memory( sizeof(size_t) * 2 * FANOUT_PASS2_ASYMM * FANOUT_PASS1_ASYMM, CACHELINE_SIZE);
	memset_localize(r_hist_2nd_pass, sizeof(size_t) * 2 * FANOUT_PASS2_ASYMM * FANOUT_PASS1_ASYMM);
	// memset_localize(s_hist_2nd_pass, sizeof(size_t) * 2 * FANOUT_PASS2_ASYMM * FANOUT_PASS1_ASYMM);

#ifdef USE_SWWCB_OPTIMIZED_RADIX_CLUSTER
#ifdef USE_SWWCB_OPTIMIZED_RADIX_CLUSTER_ON_NVM
	r_swwcb_2nd_pass_mem = new_alloc_nvm(sizeof(swwcb_t) * FANOUT_PASS2_ASYMM * FANOUT_PASS1_ASYMM);
	// s_swwcb_2nd_pass_mem = new_alloc_nvm(sizeof(swwcb_t) * FANOUT_PASS2_ASYMM * FANOUT_PASS1_ASYMM);
	pmem_memset_persist(r_swwcb_2nd_pass_mem, 0, sizeof(swwcb_t) * FANOUT_PASS2_ASYMM * FANOUT_PASS1_ASYMM);
	// pmem_memset_persist(s_swwcb_2nd_pass_mem, 0, sizeof(swwcb_t) * FANOUT_PASS2_ASYMM * FANOUT_PASS1_ASYMM);
#else /* USE_SWWCB_OPTIMIZED_RADIX_CLUSTER_ON_NVM */
	r_swwcb_2nd_pass_mem = alloc_aligned_dram(sizeof(swwcb_t) * FANOUT_PASS2_ASYMM * FANOUT_PASS1_ASYMM, CACHELINE_SIZE);
	// s_swwcb_2nd_pass_mem = alloc_aligned_dram(sizeof(swwcb_t) * FANOUT_PASS2_ASYMM * FANOUT_PASS1_ASYMM, CACHELINE_SIZE);
	memset(r_swwcb_2nd_pass_mem, 0, sizeof(swwcb_t) * FANOUT_PASS2_ASYMM * FANOUT_PASS1_ASYMM);
	// memset(s_swwcb_2nd_pass_mem, 0, sizeof(swwcb_t) * FANOUT_PASS2_ASYMM * FANOUT_PASS1_ASYMM);
#endif /* USE_SWWCB_OPTIMIZED_RADIX_CLUSTER_ON_NVM */
#endif /* USE_SWWCB_OPTIMIZED_RADIX_CLUSTER */
	...
	skew_queue = task_queue_init(FANOUT_PASS1_ASYMM);
#endif /* SKEW_HANDLING */
	...
	phj_asymm_radix_arg_bw_reg_t args[max_thread_num];
	...
	for (int idx = 0; idx < BUILDPART_THREAD_NUM; idx ++) {
		...
		rv = pthread_create(&threadpool[idx], &attr, prj_asymm_radix_thread_buildpart_bw_reg, args+idx);
		if (rv){
	...
	size_t intermediate_size;
#if TUPLE_T_SIZE > 16
	if (f_join == asymm_radix_sc_bw_reg_join) {
	...
	for (int idx = 0; idx < PROBEJOIN_THREAD_NUM; idx ++) {
		...
		rv = pthread_create(&threadpool[idx], &attr, prj_asymm_radix_thread_probejoin_bw_reg, args+idx);
		if (rv){
			printf("ERROR; return code from pthread_create() is %d\n", rv);
			exit(EXIT_FAILURE);
		}
	...
	dealloc_memory(r_hist_2nd_pass, sizeof(size_t) * 2 * FANOUT_PASS2_ASYMM * FANOUT_PASS1_ASYMM);
	// dealloc_memory(s_hist_2nd_pass, sizeof(size_t) * 2 * FANOUT_PASS2_ASYMM * FANOUT_PASS1_ASYMM);

#ifdef USE_SWWCB_OPTIMIZED_RADIX_CLUSTER
#ifdef USE_SWWCB_OPTIMIZED_RADIX_CLUSTER_ON_NVM
	pmem_unmap(r_swwcb_2nd_pass_mem, sizeof(swwcb_t) * FANOUT_PASS2_ASYMM * FANOUT_PASS1_ASYMM);
	// pmem_unmap(s_swwcb_2nd_pass_mem, sizeof(swwcb_t) * FANOUT_PASS2_ASYMM * FANOUT_PASS1_ASYMM);
#else /* USE_SWWCB_OPTIMIZED_RADIX_CLUSTER_ON_NVM */
...
void phj_asymm_radix_bw_reg_bc(const datameta_t datameta, timekeeper_t * const timekeeper) {
	asymm_radix_join_init_run_bw_reg(datameta, timekeeper, asymm_radix_bc_bw_reg_join);
}

void phj_asymm_radix_bw_reg_sc(const datameta_t datameta, timekeeper_t * const timekeeper) {
	asymm_radix_join_init_run_bw_reg(datameta, timekeeper, asymm_radix_sc_bw_reg_join);
}

void phj_asymm_radix_bw_reg_hr(const datameta_t datameta, timekeeper_t * const timekeeper) {
	asymm_radix_join_init_run_bw_reg(datameta, timekeeper, asymm_radix_hr_bw_reg_join);
}

#if TUPLE_T_SIZE <= AVX512_SIZE
void phj_asymm_radix_bw_reg_hro(const datameta_t datameta, timekeeper_t * const timekeeper) {
	asymm_radix_join_init_run_bw_reg(datameta, timekeeper, asymm_radix_hro_bw_reg_join);
}
#endif /* TUPLE_T_SIZE <= AVX512_SIZE */
