...
void * test_part(void *param) {
	...
	size_t mem_size = (size_t) PAGE_SIZE * (1 << NUM_RADIX_BITS);
	...
	size_t max_range_idx = (1 << NUM_RADIX_BITS) - 1;
	...
	for (size_t idx = 0; idx < outer_loop_num; idx ++) {
		...
		void *dest_addr = dest + (size_t) PAGE_SIZE * dest_idx;
#ifdef USE_NVM
...
double test_tlb() {
	...
	printf("SWWCB_SIZE: %d\tPART_NUM: %d\total elapsed time: %.9f\n", 
		SWWCB_SIZE, 1 << NUM_RADIX_BITS,
		diff_sec(args[0].clock_start, args[0].clock_end)
	);
	deldir(work_dir);
...
int main(int argc, char **argv) {
	...
	for (int t = 0; t < TRIAL_NUM; t ++) {
		total_elapsed_time += test_tlb();
	}
	printf("general elapsed time: %d\t%d\t%.8f\n", SWWCB_SIZE, 1<<NUM_RADIX_BITS, total_elapsed_time/TRIAL_NUM);
