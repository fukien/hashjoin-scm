...
if __name__ == "__main__":
	...
	plt.title("dupfk {}".format(mem_type))
	for idx, algo in enumerate(algo_list):
		plt.bar(
			x_axis,
	...
	plt.savefig(os.path.join(FIG_PATH, "dupfk_{}.png".format(mem_type)), bbox_inches="tight", format="png")
	plt.title("", fontsize=default_fontsize)
	plt.savefig(os.path.join(FIG_PATH, "dupfk_{}.pdf".format(mem_type)), bbox_inches="tight", format="pdf")
	os.system("pdftops -eps {} {}".format(
			os.path.join(FIG_PATH, "dupfk_{}.pdf".format(mem_type)),
			os.path.join(FIG_PATH, "dupfk_{}.eps".format(mem_type))
		)
	)
