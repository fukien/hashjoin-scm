...
LOG_PATH = "../../logs/pagefault-persist-logs/"
FIG_PATH = "../../figs/pagefault-persist-figs/"
...
if __name__ == "__main__":
	...
	plt.title("persist {}".format(mem_type))
	...
	for idx, algo in enumerate(algo_list):
		...
		filepath = os.path.join(
			LOG_PATH, 
			"{}_regular_{}.log".format(algo2logname[algo], mem_type)
		)
		...
		filepath = os.path.join(
			LOG_PATH, 
			"{}_clflush_{}.log".format(algo2logname[algo], mem_type)
		)
		buildpart_time, probejoin_time, _ =  get_data_from_log(filepath)
		buildpart_time_list.append(buildpart_time)
		probejoin_time_list.append(probejoin_time)
		...
		filepath = os.path.join(
			LOG_PATH, 
			"{}_clflushopt_{}.log".format(algo2logname[algo], mem_type)
		)
		buildpart_time, probejoin_time, _ =  get_data_from_log(filepath)
		buildpart_time_list.append(buildpart_time)
		probejoin_time_list.append(probejoin_time)
		...
		filepath = os.path.join(
			LOG_PATH, 
			"{}_ntstore_{}.log".format(algo2logname[algo], mem_type)
		)
		buildpart_time, probejoin_time, _ =  get_data_from_log(filepath)
		buildpart_time_list.append(buildpart_time)
		probejoin_time_list.append(probejoin_time)
	...
	plt.savefig(os.path.join(FIG_PATH, "persist_{}.png".format(mem_type)), bbox_inches="tight", format="png")
	# plt.title("", fontsize=default_fontsize)
	# plt.savefig(os.path.join(FIG_PATH, "persist_{}.pdf".format(mem_type)), bbox_inches="tight", format="pdf")
	# os.system("/Applications/Inkscape.app/Contents/MacOS/inkscape {} --export-eps={}".format(
	# 		os.path.join(FIG_PATH, "persist_{}.pdf".format(mem_type)),
	# 		os.path.join(FIG_PATH, "persist_{}.eps".format(mem_type))
	# 	)
	# )
