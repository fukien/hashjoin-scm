...
LOG_PATH = "../../logs/nphj-prefetching-logs/"
		...
		while True:
			...
			if flag == "papi":
				if line.startswith("[agg_PAPI]") and event in line:
					line = line.strip().split("\t")
					buildpart_counter_list.append( round4decimals(line[-2].split()[-1])/(10**6) )	# buildpart
					probejoin_counter_list.append( round4decimals(line[-1].split()[-1])/(10**6) )	# probejoin
					total_counter_list.append( round4decimals(line[-3].split()[-1])/(10**6) )	# total
			else:	# pmwatch
